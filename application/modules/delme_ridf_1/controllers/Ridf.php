...
class RIDF extends MX_Controller {
        ...
        if ($this->input->post()) {
            ...
            if (sizeof($_post) > 0) {
                $selected = array(
                    'district_id' => $_post['district_id'],
                    'block_id' => $_post['block_id'],
                    'category_id' => $_post['fund_id']
                );
                $list = $this->model->get_requisition_list($selected['district_id'], $selected['block_id'], $selected['category_id']);
                // echo "<pre>";
                // print_r($list);exit;
            }
        }
    ...
    function requisition_entry($id = 0) {
        $session = $this->common->get_session();
        ...
        $selected = array(
            ...
            'sanctioned_cost' => '',
            'physical_progress' => '',
            'previous_claim_expenditure' => '',
            'present_claim_expenditure' => '',
            'total_claim_expenditure' => '',
            'amounts_already_claimed' => '',
            'present_claim_amount' => '',
            'ensuing_quarter_drawal' => '',
            'completion_date' => '',
            'document' => ''
        );

        if ($id > 0) {
            $row = $this->model->get_requisition_info($id);
            // print_r($row);exit;
            if ($row) {
                $selected = array(
                    'id' => $row->id,
                    ...
                    'sanctioned_cost' => $row->sanctioned_cost,
                    'ridf_loan' => $row->ridf_loan,
                    'physical_progress' => $row->physical_progress,
                    'previous_claim_expenditure' => $row->previous_claim_expenditure,
                    'present_claim_expenditure' => $row->present_claim_expenditure,
                    'total_claim_expenditure' => $row->total_claim_expenditure,
                    'amounts_already_claimed' => $row->amounts_already_claimed,
                    'present_claim_amount' => $row->present_claim_amount,
                    'ensuing_quarter_drawal' => $row->ensuing_quarter_drawal,
                    'completion_date' => $row->completion_date,
                    'document' => $row->document
                );
                $block_list = $this->common->get_block_list($selected['district_id']);
                // print_r($road_list);exit;
            }
        }
        ...
        $this->data['district'] = json_encode($district_list);
        $this->data['block'] = json_encode($block_list);
        $this->data['road_list'] = json_encode($this->model->get_road_list($selected['district_id'], $selected['block_id'], $selected['category_id'], $selected['agency_id']));
        $this->data['category'] = json_encode($this->model->get_category_list());
        $this->data['agency'] = json_encode($this->model->get_agency_list());
        $this->data['title'] = 'Requisition';
        $this->data['heading'] = 'Requisition - RIDF';
    ...
    function get_road_list() {
        $data = $this->input->get();
        // print_r($data);exit;
        echo json_encode($this->model->get_road_list($data['district_id'], $data['block_id'], $data['category_id'], $data['agency_id']));
    }

    function get_senction_cost() {
        $data = $this->input->get();
        // print_r($data);exit;
        echo json_encode($this->model->get_senction_cost($data['work_id']));
    }

    function requisition_save() {
        $post_data = $this->input->post();
        $id = $this->model->requisition_save($post_data);
        $this->upload_requisition($id);
        // print_r($id);exit;
        $this->session->set_flashdata('message', array('type' => 'success', 'message' => 'Data added successfully'));
        redirect('ridf/requisition');
        // if (!$result['status']) {
        //     // Show error message and redirect or load view
        //     $this->session->set_flashdata('message', array('type' => 'danger', 'message' => $result['message']));
        //     redirect('ridf/requisition');
        // } else {
        //     $this->session->set_flashdata('message', array('type' => 'success', 'message' => 'Data added successfully'));
        //     redirect('ridf/requisition');
    ...
    function upload_requisition($id) {
        // print_r($id['insert_id']);exit;
        $ext = rand(1, 1000000);
        ...
        if (!$this->upload->do_upload('userfile')) {
            $error = array('error' => $this->upload->display_errors());
            // print_r($error);exit;
        } else { //echo 'am here'; exit;
            $image = array('upload_data' => $this->upload->data());
            $old_file = $path . $image['upload_data']['file_name'];
            $new_file = $path . $id['insert_id'] . $image['upload_data']['file_ext'];
            rename($old_file, $new_file);

            $input = array(
                'document' => $new_file
            );
            $this->db->where('id', $id['insert_id']);
