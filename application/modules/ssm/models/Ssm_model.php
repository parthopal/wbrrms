...
class SSM_Model extends CI_Model
{
    ...
    function get_dashboard_count()
    {
        ...
        $this->db->select('sum(approved_scheme) as approved_scheme, sum(approved_length) as length, sum(approved_amount) as sanctioned_amount, sum(tender_invited) as tender_invited,  '
            . 'sum(tender_matured) as tender_matured, sum(tm_length) as lender_length, sum(tm_amount) as tender_amount, sum(wo_issued) as wo_no, sum(wo_length) as wo_length, '
            . 'sum(wo_amount) as wo_amount,(sum(progress_25) + sum(progress_50) + sum(progress_75) + sum(progress_99)) as ongoing,'
        ...
        $query = $this->db->get(SSM_REPORT . ' s');
        return $query->row();
    }
    ...
    function get_tender_and_wo_count()
    {
        ...
        $where .= $session['block_id'] > 0 ? ' and block_id in (' . $session['block_id'] . ')' : '';
        $where .= $this->filter_with_agency();
        $sql = "SELECT COUNT(id) as tender_matured, ifnull(SUM(length),0) as tender_matured_length, SUM(cost) as tender_amount FROM ssm "
            . "WHERE isactive = 1 AND tender_status = 2 " . $where;
        $query = $this->db->query($sql);
        $tender_matured_count = $query->row();

        $sql = "SELECT COUNT(s.id) as work_order, ifnull(SUM(s.length),0) as work_order_length, SUM(wo.awarded_cost) as sanctioned_amount FROM ssm s JOIN ssm_wo wo ON wo.ssm_id = s.id "
        ...
        $sql = "SELECT COUNT(s.id) as work_progress, ifnull(SUM(s.length),0) as work_progress_length, SUM(wo.awarded_cost) as sanctioned_amount "
            . "FROM ssm s JOIN ssm_wo wo ON wo.ssm_id = s.id WHERE s.isactive = 1 AND s.wo_status = 2 AND s.pp_status = 5 " . $where;
        $query = $this->db->query($sql);
    ...
    function get_survey_list($district_id, $status = 0)
    {
        ...
        $this->db->select('s.id, d.name as district, '
            ...
            . 'b.name as block, gp.name as gp, s.village, s.ref_no, s.name, s.agency, s.work_type, s.road_type, s.survey_status, '
            . 's.survey_lot_no, s.dm_lot_no, s.se_lot_no, s.isactive, ni.remarks');
        $this->db->where(array(
        ...
        $this->db->join(SSM_NOT_IMPLEMENTED_LOG . ' ni', 'ni.ssm_id=s.id', 'left');
        $this->db->order_by('b.name');
        $query = $this->db->get(SSM . ' s'); //echo $this->db->last_query(); exit;
        return $query->result();
    }
    ...
    function get_ac_list($district_id = 0)
    {
        ...
        $this->db->join(SSM . ' s', 's.ac_id=a.id');
    ...
    function save($data)
    {
        ...
        if ($session['role_id'] < 4) {
            ...
            //            $input['work_type'] = $data['work_type'];
            //            $input['road_type'] = $data['road_type'];
        ...
        if ($flag) {
            ...
            if ($data['isapproved'] > 0) {
                $ref_no = str_pad($data['district_id'] . rand(0, 999), 5, '0', STR_PAD_LEFT) . '/' . $data['agency'] . '/' . $district_name . '/RASTASHREE3/2024';
                $this->db->where(array(
                    'ref_no' => $ref_no
                ));
                $query = $this->db->get(SSM);
                ...
                $ref_no = 'TMP/' . str_pad($data['district_id'] . rand(0, 999), 5, '0', STR_PAD_LEFT) . '/' . $data['agency'] . '/' . $district_name . '/SSM/2024';
            }
            $input = array(
                'ref_no' => $ref_no
            );
    ...
    function survey_save($data)
    {
        ...
        $this->db->insert(SSM_SURVEY_LOG, $input);
        $this->db->trans_complete();
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return FALSE;
        ...
        foreach ($data as $d) {
            switch ($session['role_id']) {
                ...
                case 3:
                    ...
                    break;
                case 7:
                    $sql = 'SELECT agency FROM ssm WHERE id=' . $d;
                    $query = $this->db->query($sql);
    ...
    {
        $session = $this->common->get_session();
        $this->db->select('s.id, d.name as district, concat(ac.no, " ", ac.name, " ", IF((ac.reserved is null or ac.reserved=""), "", concat("(", ac.reserved, ")"))) as ac, b.name as block, gp.name as gp, s.ref_no, s.name, s.length, '
    ...
    function not_traceable_save($data)
    {
        ...
        $this->db->insert(SSM_NOT_IMPLEMENTED_LOG, $input);
        $this->db->trans_complete();
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return FALSE;
    ...
    function get_tender_list($district_id, $block_id, $ac_id)
    {
        ...
        $this->db->join('(SELECT * FROM ssm_tender_log tl JOIN (SELECT ssm_id as c_ssm_id, MAX(id) as max_id FROM ssm_tender_log GROUP BY ssm_id) mtl ON tl.id = mtl.max_id)' . ' t', 's.id=t.ssm_id', 'left');
        $this->db->join(DIVISION . ' d', 's.district_id=d.id');
        $this->db->join(DIVISION . ' gp', 's.gp_id=gp.id');
        $this->db->join(DIVISION . ' b', 's.block_id=b.id');
        $this->db->join(AC . ' ac', 's.ac_id=ac.id');
    ...
    function tender_save($data)
    {
        ...
        // var_dump($data);exit;
        $this->db->insert(SSM_TENDER_LOG, $input);
    ...
    {
        $this->db->select('wo.id, sr.id as ssm_id, sr.name, sr.district_id, wo.wo_no, '
        ...
        $this->db->join(SSM_WO . ' wo', 'wo.ssm_id=sr.id and wo.isactive=1', 'left');
        $query = $this->db->get(SSM . ' sr');
        return $query->row();
    }
    ...
    function get_wo_list($district_id = 0, $block_id = 0, $ac_id = 0)
    {
        ...
        $this->db->select('d.name as district, concat(ac.no, " ", ac.name, " ", IF((ac.reserved is null or ac.reserved=""), "", concat("(", ac.reserved, ")"))) as ac, b.name as block, s.name,s.agency, s.tender_number, '
            ...
            . 'wo.id, s.id as ssm_id, wo.wo_no, date_format(wo.wo_date, "%d/%m/%Y") as wo_date, wo.contractor, '
        ...
        $this->db->join(DIVISION . ' d', 's.district_id=d.id');
        $this->db->join(DIVISION . ' b', 's.block_id=b.id');
        $this->db->join(SSM_WO . ' wo', 'wo.ssm_id=s.id', 'left');
    ...
    {
        $this->db->trans_start();
        $ssm_id = isset($data['ssm_id']) ? $data['ssm_id'] : 0;
        ...
        if ($ssm_id > 0) {
            ...
            if ($query->num_rows() > 0) {
                ...
                $sql = 'UPDATE ssm SET pe_user_id=' . $user_id . ' WHERE id=' . $ssm_id;
                $this->db->query($sql);
            } else {
    ...
    function get_wp_list($user_id, $status)
    {
        ...
        $sql = 'SELECT s.id, b.name as block, g.name as gp, s.ref_no, s.name, s.wo_start_date, s.physical_progress, s.progress_remarks, s.pp_status '
            . 'FROM ssm s JOIN division b ON s.block_id=b.id JOIN division g ON s.gp_id=g.id ' . $where;
        $query = $this->db->query($sql);
    ...
    function get_rpt_state_summary($ac_id = 0)
    {
        ...
        $sql = 'select district, sum(approved_scheme) as approved_scheme, sum(approved_length) as approved_length, sum(approved_amount) as approved_amount,
            ...
            (sum(progress_25_amount) + sum(progress_50_amount) + sum(progress_75_amount) + sum(progress_99_amount)) as ongoing_amount,
            sum(progress_100) as completed, sum(progress_100_length) as completed_length, sum(progress_100_amount) as completed_amount
            from ssm_report ' . $where . ' group by district order by district ';
        $query = $this->db->query($sql);
        return $query->result();
    }
    ...
    function get_rpt_agency_progress($ac_id = 0)
    {
        ...
        $where = $session['district_id'] > 0 ? ' where district_id in (' . $session['district_id'] . ')' : '';
        $where .= $session['block_id'] > 0 ? ' and block_id in (' . $session['block_id'] . ')' : '';
        // $where .= $ac_id > 0 ? (strlen($where) > 0 ? ' and' : 'WHERE ''. . 'category_id=' . $category_id : '';
        $where .= $ac_id > 0 ? (strlen($where) > 0 ? ' and' : 'WHERE ') . ' ac_id=' . $ac_id : '';
        $where .= $this->filter_with_agency();
        $sql = 'select district, agency, sum(approved_scheme) as approved_scheme, sum(approved_length) as approved_length, sum(approved_amount) as approved_amount,
            ...
            (sum(progress_25_amount) + sum(progress_50_amount) + sum(progress_75_amount) + sum(progress_99_amount)) as ongoing_amount,
            sum(progress_100) as completed, sum(progress_100_length) as completed_length, sum(progress_100_amount) as completed_amount
            from ssm_report ' . $where . ' group by district, agency order by district, agency ';
        $query = $this->db->query($sql);
        //echo $this->db->last_query($query); exit;
        return $query->result();
    }
    ...
    {
        $session = $this->common->get_session();
        $where = $session['district_id'] > 0 ? (strlen($where) > 0 ? ' AND ' : '') . 'district_id in (' . $session['district_id'] . ')' : '';
        $where .= $session['block_id'] > 0 ? (strlen($where) > 0 ? ' AND ' : '') . 'block_id in (' . $session['block_id'] . ')' : '';
        $where .= $ac_id > 0 ? (strlen($where) > 0 ? ' AND ' : '') . 'ac_id=' . $ac_id : '';
        $where .= $road_type != '' ? (strlen($where) > 0 ? ' AND ' : '') . 'road_type="'. $road_type . '"' : '';
        $where .= $this->filter_with_agency();
        ...
        $sql = 'select district, agency, road_type, sum(approved_scheme) as approved_scheme, sum(approved_length) as approved_length, sum(approved_amount) as approved_amount,
            ...
            (sum(progress_25_amount) + sum(progress_50_amount) + sum(progress_75_amount) + sum(progress_99_amount)) as ongoing_amount,
            sum(progress_100) as completed, sum(progress_100_length) as completed_length, sum(progress_100_amount) as completed_amount
            from ssm_report ' . $where . ' group by district, agency, road_type order by district, agency ';
        $query = $this->db->query($sql);
        //echo $this->db->last_query($query);exit;
        return $query->result();
    }
    ...
    function get_rpt_work_type_progress($work_type = '', $ac_id = 0)
    {
        ...
        $where .= $session['block_id'] > 0 ? (strlen($where) > 0 ? ' AND ' : '') . 'block_id in (' . $session['block_id'] . ')' : '';
        $where .= $ac_id > 0 ? (strlen($where) > 0 ? ' AND ' : '') . 'ac_id=' . $ac_id : '';
        $where .= $work_type != '' ? (strlen($where) > 0 ? ' AND ' : '') . 'work_type="' . $work_type . '"' : '';
        $where .= $this->filter_with_agency();
		$where = strlen($where) > 0 ? 'WHERE ' . $where : '';
        $sql = 'select district, agency, work_type,sum(approved_scheme) as approved_scheme, sum(approved_length) as approved_length, sum(approved_amount) as approved_amount,
            ...
            (sum(progress_25_amount) + sum(progress_50_amount) + sum(progress_75_amount) + sum(progress_99_amount)) as ongoing_amount,
            sum(progress_100) as completed, sum(progress_100_length) as completed_length, sum(progress_100_amount) as completed_amount
            from ssm_report ' . $where . ' group by district, agency, work_type order by district, agency ';
        $query = $this->db->query($sql);
		//echo $this->db->last_query($query);exit;
        return $query->result();
    }
    ...
    function get_rpt_ps_work_status($district_id = 0, $block_id = 0, $ac_id = 0)
    {
        ...
        $sql = 'select district, block, agency, sum(approved_scheme) as approved_scheme, sum(approved_length) as approved_length, sum(approved_amount) as approved_amount,
            ...
            from ssm_report ' . (strlen($where) > 0 ? 'where ' . $where : '') . ' group by district,block, agency order by district, agency ';
        $query = $this->db->query($sql);
        return $query->result();
    }
    ...
    function get_rpt_work_progress($district_id = 0, $block_id = 0, $wp_id = 0, $ac_id = 0)
    {
        ...
        $sql = 'select s.id, d.name as district, b.name as block, s.name, s.agency, s.length, wo.awarded_cost, s.road_type, s.work_type, '
            . 's.wo_start_date, s.pp_status, s.physical_progress,s.image1,s.image2,s.image3,  wp.wp_date as last_work_date, wp.date_count as inspection_count from ssm s '
            . 'join division d on s.district_id=d.id join division b on s.block_id=b.id join ssm_wo wo on wo.ssm_id=s.id '
            . 'LEFT JOIN (SELECT ssm_id, max(created) As wp_date,wo_start_date,count(created) as date_count FROM ssm_progress GROUP BY ssm_id) wp ON s.id = wp.ssm_id AND s.wo_start_date = wp.wo_start_date '
    ...
    function get_rpt_work_progress_details($ssm_id)
    {
        ...
        sp.image3, sp.progress_remarks FROM ssm_progress sp join ssm s on sp.ssm_id=s.id WHERE sp.ssm_id=' . $ssm_id . ' ORDER BY sp.created';
        $query = $this->db->query($sql);
        return $query->result();
    }
    ...
    function get_rpt_approval_progress($ac_id = 0)
    {
        ...
        $sql = 'SELECT d.name as district, s.agency, count(s.id) as total_work, sum(s.length) as total_length, '
            ...
            . 'FROM ssm s JOIN division d ON s.district_id=d.id ' . $where . ' '
