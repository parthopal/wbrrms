...
class Capex_Model extends CI_Model {
    ...
    function get_scheme_list($district_id, $category_id, $type_id) {
        $sql = 'SELECT r.id, r.name, d.name as district, b.name as block, ac.id as ac_id, ac.name as ac, a.name as agency, r.category_id, r.scheme_id, r.length, '
                . 'r.sanctioned_cost, r.admin_date, r.admin_no, r.road_type_name as work_type, r.length, r.isactive, r.note, r.survey_status, r.tender_status, r.wo_status from ridf as r '
...
//        $db = $this->load->database('rpt', TRUE);
    ...
    function get_bridge_list($district_id, $block_id, $category_id) {
        ...
        $sql = 'SELECT rb.id, rb.name, d.name as district, b.name as block, rb.agency, date_format(rb.aot_date, "%d/%m/%Y") as aot_date, date_format(rb.wo_date, "%d/%m/%Y") as wo_date, date_format(rb.completion_date, "%d/%m/%Y") as complete_date, sc.subcategory as funding, rb.isactive from capex_bridge as rb '
    ...
    function get_bridge_info($id) {
        ...
        $where = $session['district_id'] > 0 ? 'and district_id in (' . $session['district_id'] . ')' : '';
        $where .= $session['block_id'] != 0 ? ' and block_id in (' . $session['block_id'] . ')' : '';
        $sql = 'SELECT rb.id, rb.district_id, rb.block_id, rb.category_id, rb.name, rb.agency, rb.aot_date as aot_date, rb.wo_date as wo_date, rb.completion_date as complete_date, rb.isactive from capex_bridge as rb '
    ...
    function bridge_save($data) {
        if ($data['block_id'] == 0) {
            ...
            $sql = 'SELECT id from capex_bridge where id=' . $id;
            $query = $this->db->query($sql);
            ...
            if ($id > 0) {
                ...
                $this->db->update(CAPEX_BRIDGE_QM, $input_qm_update);
                $this->db->where('bridge_id', $bridge_id);
                $this->db->update(CAPEX_BRIDGE_INSPECTION, $input_qm_update);
            } else {
                $this->db->insert(CAPEX_BRIDGE_QM, $input_qm);
                $this->db->insert(CAPEX_BRIDGE_INSPECTION, $input_qm);
            }
            $this->db->trans_complete();
            if ($this->db->trans_status() === FALSE) {
                $this->db->trans_rollback();
    ...
    function get_bridge_qm_list($month, $year) {
        ...
        $where .= $session['role_id'] == 23 ? ' and qm.sqm_id=' . $session['user_id'] : '';
        $sql = 'SELECT CONCAT(u.id, "-", qm.month, "-", qm.year,"-",qm.bridge_id) as id, u.name as sqm, u.mobile, qm.status,s.name, qm.physical_progress, qm.financial_progress,
            ...
            FROM capex_bridge_qm as qm JOIN division d ON qm.district_id=d.id JOIN capex_bridge s ON s.id=qm.bridge_id JOIN division b ON s.block_id=b.id JOIN user u ON u.id=qm.sqm_id
    ...
    function get_bridge_qm_scheme_list($district_id, $sqm_id = 0, $month = 0, $year = 0) {
        ...
        $sql = 'SELECT s.id, d.name as district, b.name as block, s.name, s.agency, s.wo_date as wo_start_date, q.physical_progress, q.inspection_date, q.overall_grade, ifnull(q.id, 0) as selected
            FROM capex_bridge s JOIN division d ON s.district_id=d.id JOIN division b ON s.block_id=b.id
            LEFT JOIN capex_bridge_qm q ON s.id=q.bridge_id and q.sqm_id=' . $sqm_id . ' and q.month=' . $month . ' and q.year=' . $year . ' and q.isactive=1
            LEFT JOIN (SELECT sq.bridge_id, sq.inspection_date, sq.overall_grade FROM ridf_bridge_qm sq WHERE sq.inspection_date=(SELECT max(sqi1.inspection_date) from capex_bridge_qm as sqi1 join capex_bridge_qm sq1 on sqi1.sqm_id=sq1.id WHERE sq1.bridge_id=sq.bridge_id)) as qm ON qm.bridge_id=s.id
            LEFT JOIN (SELECT DISTINCT bridge_id, count(bridge_id) as cnt FROM capex_bridge_qm GROUP BY bridge_id) as total ON total.bridge_id=s.id
    ...
    function bridge_qm_save($data) {
        ...
        foreach ($data['chk'] as $row) {
            ...
            $query = $this->db->get(CAPEX_BRIDGE_QM);
            if ($query->num_rows() > 0) {
                ...
                $input['created'] = date('Y-m-d');
                $this->db->insert(CAPEX_BRIDGE_QM, $input);
            }
        }
        $this->db->trans_complete();
        if ($this->db->trans_status() === FALSE) {
    ...
    function get_bridge_qm_caption($qm_id, $bridge_id) {
        ...
        $this->db->join(CAPEX_BRIDGE . ' b', 'qm.bridge_id=b.id');
        $query = $this->db->get(CAPEX_BRIDGE_QM . ' qm');
        return $query->result();
    }
    ...
    function get_bridge_qm_image_list($id, $bridge_id) {
        ...
        $this->db->join(CAPEX_BRIDGE_QM_IMAGE . ' i', 'i.inspection_id=hd.id');
        $this->db->order_by('i.seq_id');
        $query = $this->db->get(CAPEX_BRIDGE_QM . ' hd');
        return $query->result();
    }
    ...
    function bridge_qm_save_image($i, $id, $desc) {
        ...
        $query = $this->db->get(CAPEX_BRIDGE_QM_IMAGE);
        ...
        if ($image_id > 0) {
            ...
            $input['created'] = date('Y-m-d');
            $this->db->insert(CAPEX_BRIDGE_QM_IMAGE, $input);
    ...
    function get_bridge_inspection_scheme_list($district_id, $sqm_id = 0, $month = 0, $year = 0) {
        ...
        $sql = 'SELECT s.id, d.name as district, b.name as block, s.name, s.agency, q.length,q.fundation, q.length, q.width, q.hfl, q.ofl, q.obstruction, q.traffic_category, q.lbl, q.proposed_bridge, q.super_structure, q.linear_waterway, q.linear_water_provided, total.cnt, ifnull(q.id, 0) as selected
            FROM capex_bridge s JOIN division d ON s.district_id=d.id JOIN division b ON s.block_id=b.id
            LEFT JOIN capex_bridge_inspection q ON s.id=q.bridge_id and q.sqm_id=' . $sqm_id . ' and q.month=' . $month . ' and q.year=' . $year . ' and q.isactive=1
            LEFT JOIN (SELECT sq.bridge_id, sqi.inspection_date FROM capex_bridge_inspection sq JOIN capex_bridge_inspection sqi ON sqi.sqm_id=sq.id WHERE sqi.inspection_date=(SELECT max(sqi1.inspection_date) from capex_bridge_inspection as sqi1 join capex_bridge_qm sq1 on sqi1.sqm_id=sq1.id WHERE sq1.bridge_id=sq.bridge_id)) as qm ON qm.bridge_id=s.id
            LEFT JOIN (SELECT DISTINCT bridge_id, count(bridge_id) as cnt FROM capex_bridge_inspection GROUP BY bridge_id) as total ON total.bridge_id=s.id
    ...
    function bridge_inspection_save($data) {
        ...
        foreach ($data['chk'] as $row) {
            ...
            $query = $this->db->get(CAPEX_BRIDGE_INSPECTION);
            if ($query->num_rows() > 0) {
                $this->db->where('id', $query->row()->id);
                $this->db->update(CAPEX_BRIDGE_INSPECTION, $input);
            } else {
                $input['created'] = date('Y-m-d');
                $this->db->insert(CAPEX_BRIDGE_INSPECTION, $input);
            }
        }
        $this->db->trans_complete();
        if ($this->db->trans_status() === FALSE) {
    ...
    function get_bridge_inspection_list($month, $year) {
        ...
        $sql = 'SELECT CONCAT(u.id, "-", qm.month, "-", qm.year,"-",qm.bridge_id) as id, u.name as sqm, u.mobile, qm.status,s.name,s.agency,u.mobile as sqm_mobile,
            d.name as district, b.name as block, bridge_id as total
            FROM capex_bridge_inspection as qm JOIN capex_bridge s ON s.id=qm.bridge_id JOIN division d ON qm.district_id=d.id JOIN division b ON s.block_id=b.id 
    ...
    function get_bridge_inspection_caption($qm_id, $bridge_id) {
        ...
        $this->db->join(CAPEX_BRIDGE . ' b', 'i.bridge_id=b.id');
        $query = $this->db->get(CAPEX_BRIDGE_INSPECTION . ' i');
        return $query->result();
    }
    ...
    function get_bridge_inspection_image_list($id, $bridge_id) {
        ...
        $this->db->join(CAPEX_BRIDGE_INSPECTION_IMAGE . ' i', 'i.inspection_id=hd.id');
        $this->db->order_by('i.seq_id');
        $query = $this->db->get(CAPEX_BRIDGE_INSPECTION . ' hd');
        return $query->result();
    }
    ...
    function bridge_inspection_save_image($i, $id, $desc) {
        ...
        $query = $this->db->get(CAPEX_BRIDGE_INSPECTION_IMAGE);
        ...
        if ($image_id > 0) {
            ...
            $input['created'] = date('Y-m-d');
            $this->db->insert(CAPEX_BRIDGE_INSPECTION_IMAGE, $input);
            $image_id = $this->db->insert_id();
        }
        $this->db->trans_complete();
        if ($this->db->trans_status() === FALSE) {
