...
//                $('#cost_involved').attr('readonly', true);
    ...
    function save() {
        ...
        $.ajax({
            ...
            type: 'get',
            data: {id: id, length: $('#length').val(), cost_involved: $('#cost_involved').val(), pp: $('#pp').val(), fp: $('#fp').val(), bp: $('#bp').val(), bd: $('#bd').val(), isupdated: $('#isupdated').val()},
            dataType: "json"
        }).done(function (data) {
            $('#view').show();
            $('#entry').hide();
    ...
    function date_complete(id) {
        ...
        // console.log(date);
        // console.log(id);
        if(date.split("/").reverse().join("-") > start_date.split("/").reverse().join("-") && date_format.test(date)){
                $.ajax({
                    url: baseURL + "/srrp/update_completion_date",
                    type: "get",
                    data: {
                        id: id,
                    ...
                    $.notify("<span style=color:#00FF00;font-size:150% >Date Save Successfully</span>");
                    setTimeout(function(){
                    location.reload();
                    }, 3000);
                });               
            }else{
                $.notify("<span style=color:#FFA500;font-size:120% >Please enter date format correctly OR Complete Date Greater-than Start Date</span>");
            }
    //     }else{
    //         $.notify("<span style=color:#FFA500;font-size:145% > Enter Complete Date Greater-than Start Date  </span>");
    // }
    ...
    function update_wo_date(id){      
        ...
        console.log(wo_date);
        if(wo_date != '' && date_format.test(wo_date) ){
                $.ajax({
                    url: baseURL + "/srrp/update_wo_date",
                    type: "get",
                    data: {
                        id: id,
                    ...
                    $.notify("<span style=color:#00FF00;font-size:150% >Save Successfully</span>");
                });
                setTimeout(function(){
                    location.reload();
                    }, 3000);
        }else{
            $.notify("<span style=color:#FFA500;font-size:145% >Please enter date format correctly </span>");
    }
    }
