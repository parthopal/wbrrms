...
class SRRP_Model extends CI_Model {
    ...
    function get_dashboard_count() {
        ...
        $query = $this->db->get(SRRP_REPORT . ' s');
        return $query->row();
    }
    ...
    function get_tender_and_wo_count() {
        ...
        $where .= $session['block_id'] > 0 ? ' and block_id in (' . $session['block_id'] . ')' : '';
        $where .= $this->filter_with_agency();
        $sql = "SELECT COUNT(id) as tender_matured, SUM(length) as tender_matured_length, SUM(cost) as tender_amount FROM srrp WHERE isactive = 1 AND tender_status = 2 " . $where;
        $query = $this->db->query($sql);
        $tender_matured_count = $query->row();

        $sql = "SELECT COUNT(s.id) as work_order, SUM(s.length) as work_order_length, SUM(wo.awarded_cost) as sanctioned_amount FROM srrp s JOIN srrp_wo wo ON wo.srrp_id = s.id WHERE s.isactive = 1 AND s.wo_status = 2 " . $where;
        $query = $this->db->query($sql);
        $work_order_count = $query->row();

        $sql = "SELECT COUNT(s.id) as work_progress, SUM(s.length) as work_progress_length, SUM(wo.awarded_cost) as sanctioned_amount FROM srrp s JOIN srrp_wo wo ON wo.srrp_id = s.id WHERE s.isactive = 1 AND s.wo_status = 2 AND s.pp_status = 5 " . $where;
        $query = $this->db->query($sql);
    ...
    function get_survey_list($district_id, $status = 0) {
        $session = $this->common->get_session();
        $this->db->select('s.id, d.name as district, b.name as block, gp.name as gp, s.village, s.name, s.agency, s.work_type, s.road_type, s.survey_status, s.isactive, ni.remarks, s.ref_no');
        $this->db->where(
                array(
        ...
        $this->db->join('scheme_not_implemented' . ' ni', 'ni.srrp_id=s.id', 'left');
...
################################ REPORT ########################################
    ...
    function get_rpt_work_progress($district_id = 0, $block_id = 0, $wp_id = 0, $tag = '') {
        ...
        $sql = 'select s.id, d.name as district, b.name as block, s.name, s.agency, s.length, wo.awarded_cost, s.road_type, s.work_type, '
                . 's.wo_start_date, s.pp_status, s.physical_progress,s.image1,s.image2,s.image3, wp.wp_date as last_work_date,wp.total_date as total_visit  from srrp s '
                . 'join division d on s.district_id=d.id join division b on s.block_id=b.id join srrp_wo wo on wo.srrp_id=s.id '
                . 'LEFT JOIN (SELECT srrp_id, max(created) As wp_date,count(created) as total_date, wo_start_date FROM srrp_progress GROUP BY srrp_id) wp ON s.id = wp.srrp_id AND s.wo_start_date = wp.wo_start_date '
    ...
    function get_update_status($district_id = 0, $block_id = 0, $wp_id = 0) {
        ...
        $sql = 'select s.id, d.name as district, b.name as block, s.name, s.agency, s.length,s.approved_length, wo.awarded_cost, s.road_type, s.work_type, s.cost_involved, s.work_completion_date, '
                ...
                . 'from srrp s join division d on s.district_id=d.id '
                . 'join division b on s.block_id=b.id join srrp_wo wo on wo.srrp_id=s.id '
    ...
    function update_save($data) {
        ...
        $input = array(
            ...
            'bill_paid' => $data['bp'] != '' ? $data['bp'] : 0,
            'bill_due' => $data['bd'] != '' ? $data['bd'] : 0,
    ...
    function get_rpt_updated_status($district_id = 0, $block_id = 0, $status = 0) {
        ...
        $sql = 'select s.id, d.name as district, b.name as block, s.name, s.agency, s.length,s.approved_length, wo.awarded_cost, s.road_type, s.work_type,s.cost_involved, '
                . 's.wo_start_date, s.pp_status, s.physical_progress, s.financial_progress, s.bill_paid, s.bill_due, isupdated '
                . 'from srrp s join division d on s.district_id=d.id '
                . 'join division b on s.block_id=b.id join srrp_wo wo on wo.srrp_id=s.id '
                . 'where s.survey_status=6 and s.isactive=1 ' . $where . ' order by d.name, b.name';
    ...
    function get_rpt_updated_work_summary($district_id = 0, $block_id = 0) {
        $session = $this->common->get_session();
        $where = $district_id > 0 ? ' and s.district_id in (' . $district_id . ')' : ($session['district_id'] > 0 ? ' and s.district_id in (' . $session['district_id'] . ')' : '');
        $where .= $block_id > 0 ? ' and s.block_id in (' . $block_id . ')' : ($session['block_id'] > 0 ? ' and s.block_id in (' . $session['block_id'] . ')' : '');
        $where .= $this->filter_with_agency();
    ...
    function get_completion_report(){
        $sql = 'SELECT d.name as district, b.name as block, s.name, s.approved_length, s.admin_approval_date, if(s.wo_date="1970-01-01", "", s.wo_date) as wo_date, 
                wo.completion_date, s.cost, s.physical_progress, s.financial_progress from srrp as s 
                JOIN division b ON s.block_id=b.id 
                JOIN srrp_wo wo ON wo.srrp_id=s.id 
                JOIN division d ON s.district_id = d.id 
                where s.isactive = 1 and s.agency = "SRDA" ORDER BY d.name, b.name, s.name;';
        $query = $this->db->query($sql);
    //    echo $this->db->last_query($query); exit;
        return $query->result();
    }
    ...
    //     $where = $district_id > 0 ? ' and s.district_id in (' . $district_id . ')' : ($session['district_id'] > 0 ? ' and s.district_id in (' . $session['district_id'] . ')' : '');
    //     $sql = 'SELECT s.id, s.name, d.name as district, b.name as block, s.agency, s.cost, s.length, s.wo_start_date, s.work_completion_date, s.physical_progress, s.financial_progress, s.cost_involved, s.bill_paid, s.bill_due, wo.awarded_cost FROM srrp as s 
    //             JOIN division d ON s.district_id=d.id JOIN division b ON s.block_id=b.id
    //             JOIN srrp_wo wo ON s.id = wo.srrp_id 
