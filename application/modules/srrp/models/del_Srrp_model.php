...
################################### QM #########################################
    ...
    function get_scheme_list($district_id, $sqm_id = 0, $month = 0, $year = 0) {
        $this->db->select('s.id, d.name as district, b.name as block, s.name, s.agency, s.length, s.wo_start_date, s.physical_progress,qm.inspection_date,qm.overall_grade,qm.atr_action, ifnull(q.id, 0) as selected');
        $this->db->where(array(
            's.survey_status' => 6,
            's.isactive' => 1
        ));
    ...
    function get_inspection_list($sqm_id, $month, $year) {
        ...
        $sql = 'SELECT qm.id, d.name as district, b.name as block, s.name, s.physical_progress, s.length, wo.awarded_cost, s.agency, u.name as pe, u.mobile, qm.status
            FROM srrp_qm as qm JOIN srrp s ON s.id=qm.srrp_id JOIN srrp_wo wo ON wo.srrp_id=s.id JOIN division d ON s.district_id=d.id JOIN division b ON s.block_id=b.id JOIN user u ON s.pe_user_id=u.id
