...
################################### QM #########################################
    ...
    function get_qm_list($month, $year) {
        ...
        $sql = 'SELECT CONCAT(u.id, "-", qm.month, "-", qm.year) as id, u.name as sqm,
            ...
            WHERE qm.isactive=1 and qm.month=' . $month . ' and qm.year=' . $year . $where . ' GROUP BY u.name ORDER BY u.name';
        $query = $this->db->query($sql);
        // echo $this->db->last_query(); exit;
        return $query->result();
    }
    ...
    function get_qm_details($month, $year, $sqm_id) {
        $sql = 'SELECT d.name as district, b.name as block, s.name, s.physical_progress, s.length, wo.awarded_cost, s.agency, s.image1, s.image2, s.image3
            ...
            WHERE qm.isactive=1 AND qm.month=' . $month . ' AND qm.year=' . $year . ' AND qm.sqm_id=' . $sqm_id . ' ORDER BY d.name, b.name';
        $query = $this->db->query($sql);
        //echo $this->db->last_query(); exit;
        return $query->result();
    }
...
//        $this->db->select('s.id, d.name as district, b.name as block, s.name, s.agency, s.length, s.wo_start_date, s.physical_progress,MAX(qm.inspection_date)as inspection_date,qm.overall_grade,qm.atr_action, ifnull(q.id, 0) as selected');
//        $this->db->where(array(
//            's.survey_status' => 6,
//            's.isactive' => 1
//        ));
//        $this->db->where_in('s.district_id', $district_id);
//        $this->db->join(DIVISION . ' d', 's.district_id=d.id');
//        $this->db->join(DIVISION . ' b', 's.block_id=b.id');
//        $this->db->join(SRRP_QM . ' q', 's.id=q.srrp_id and q.sqm_id=' . $sqm_id . ' and q.month=' . $month . ' and q.year=' . $year . ' and q.isactive=1', 'left');
//        $this->db->join(SRRP_QM_INSPECTION . ' qm', 's.id=qm.qm_id and qm.isactive=1', 'left');
//        $this->db->order_by('d.name, b.name, agency', 'asc');
//        $this->db->order_by('s.physical_progress', 'desc');
//        $this->db->group_by('d.name, b.name, agency');
//        $query = $this->db->get(SRRP . ' s');
        $sql = 'SELECT s.id, d.name as district, b.name as block, s.name, s.agency, s.length, s.wo_start_date, s.physical_progress, qm.inspection_date, qm.overall_grade, ifnull(qm.atr_action, 2) as atr_action, qm.cnt, ifnull(q.id, 0) as selected
            ...
            LEFT JOIN (SELECT sq.srrp_id, max(sqi.inspection_date) as inspection_date, sqi.overall_grade, COUNT(sqi.id) as cnt,
            sqi.atr_action as atr_action FROM srrp_qm sq JOIN srrp_qm_inspection sqi ON sqi.qm_id=sq.id GROUP BY sq.srrp_id) as qm ON qm.srrp_id=s.id
...
//        exit;
    ...
    function get_inspection_list($sqm_id, $month, $year) {
        $where = $sqm_id != '' ? ' AND qm.sqm_id=' . $sqm_id : ' AND qm.sqm_id=0';
    ...
    function get_inspection_report_list($sqm_id, $month, $year) {
        $where = $sqm_id != '' ? ' AND qm.sqm_id=' . $sqm_id : ' AND qm.sqm_id=0';
        $sql = 'SELECT qm.id, d.name as district, b.name as block, s.name, s.physical_progress, s.length, wo.awarded_cost, s.agency, u.name as pe, u.mobile, qm.status, sqi.inspection_date, sqi.overall_grade, sqi.document
            FROM srrp_qm as qm JOIN srrp_qm_inspection sqi ON sqi.qm_id=qm.id JOIN srrp s ON s.id=qm.srrp_id JOIN srrp_wo wo ON wo.srrp_id=s.id JOIN division d ON s.district_id=d.id JOIN division b ON s.block_id=b.id JOIN user u ON s.pe_user_id=u.id
            WHERE qm.isactive=1 AND qm.status=2 AND qm.month=' . $month . ' AND qm.year=' . $year . $where . ' ORDER BY d.name, b.name';
        $query = $this->db->query($sql);
        return $query->result();
    }
...
################################ REPORT ########################################
    ...
    function get_rpt_state_summary() {
        ...
        $sql = 'select district, sum(approved_scheme) as approved_scheme, sum(approved_length) as approved_length, sum(approved_amount) as approved_amount,
            ...
            (sum(progress_25_length) + sum(progress_50_length) + sum(progress_75_length) + sum(progress_99_length)) as ongoing_length,
