 ...
 * @since		Version 1.0,[Created: 11-Feb-2023]
 */
// ------------------------------------------------------------------------
defined('BASEPATH') or exit('No direct script access allowed');

class SRRP extends MX_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model('srrp_model', 'model');
    ...
    function entry($id = 0)
    {
        ...
        if ($id > 0) {
            $srrp = $this->model->get_scheme_info($id);
            $selected = array(
                'id' => $srrp->id,
                'tag' => $srrp->tag,
                'district_id' => $srrp->district_id,
                'block_id' => $srrp->block_id,
                'gp_id' => $srrp->gp_id,
                'village' => $srrp->village,
                'name' => $srrp->name,
                'agency' => $srrp->agency,
                'road_type' => $srrp->road_type,
                'length' => $srrp->length,
                'work_type' => $srrp->work_type,
                'cost' => $srrp->cost,
                'survey_status' => $srrp->survey_status
            );
            $subheading = $srrp->name;
        }
        $district_list = $this->common->get_district_list();
        ...
        $this->data['title'] = 'PATHASHREE-II';
        $this->data['heading'] = 'PATHASHREE-II Scheme';
    ...
    function survey()
    {
        ...
        $this->data['heading'] = 'PATHASHREE-II Scheme Inbox'; //'Pending Survey';
    ...
    function survey_entry($id)
    {
        $selected = array();
        if ($id > 0) {
            $srrp = $this->model->get_project_info($id);
            $selected = array(
                'id' => $id,
                'district' => $srrp->district,
                'block' => $srrp->block,
                'gp' => $srrp->gp,
                'name' => $srrp->name,
                'village' => $srrp->village,
    ...
    function lot()
    {
        ...
        $this->data['heading'] = 'PATHASHREE-II Batch/Lot Inbox'; // 'Scheme';
        $this->data['subheading'] = 'Scheme Details';
        $this->data['selected'] = json_encode($selected);
    ...
    function approval()
    {
        ...
        $this->data['title'] = 'Survey';
        $this->data['heading'] = 'PATHASHREE-II Vetted Scheme Inbox'; // 'Approval';
        $this->data['subheading'] = 'Scheme Details';
        $this->data['selected'] = json_encode($selected);
    ...
    function approved()
    {
        ...
        $this->data['title'] = 'Scheme';
        $this->data['heading'] = 'Approved Scheme Inbox';
        $this->data['subheading'] = 'Scheme Details';
        $this->data['selected'] = json_encode($selected);
    ...
    function not_traceable($id)
    {
        ...
        $this->data['heading'] = 'Scheme Not Implemented';
    ...
    function scheme_not_implemented()
    {
        ...
        $this->data['title'] = 'Scheme Not Implemented';
        $this->data['heading'] = 'PATHASHREE-II Not Implemented List';
        $this->data['subheading'] = 'Scheme Not Implemented';
        $this->data['selected'] = json_encode($selected);
    ...
    function delete_not_traceable($id)
    {
        ...
        redirect('srrp/scheme_not_implemented');
    }
    ...
    function tender()
    {
        ...
        $this->data['heading'] = 'PATHASHREE-II Tender';
    ...
    function tender_entry($id)
    {
        $selected = array();
        if ($id > 0) {
            $srrp = $this->model->get_tender_info($id);
            $selected = array(
                'id' => $id,
                'district' => $srrp->district,
                'block' => $srrp->block,
                ...
                'length' => $srrp->length,
                'agency' => $srrp->agency,
                'tender_number' => $srrp->tender_number,
                'tender_publication_date' => $srrp->tender_publication_date,
                'tender_status' => $srrp->tender_status,
                'bid_opeaning_date' => $srrp->bid_opeaning_date,
                'bid_closing_date' => $srrp->bid_closing_date,
                'evaluation_status' => $srrp->evaluation_status,
                'bid_opening_status' => $srrp->bid_opening_status,
                'bid_matured_status' => $srrp->bid_matured_status
            );
            if ($srrp->tender_status == 3) {
                $selected = array(
                    'id' => $id,
                    'district' => $srrp->district,
                    'block' => $srrp->block,
    ...
    function tender_benefitted_save()
    {
        ...
        if ($this->model->tender_benefitted_save($data)) {
            $this->session->set_flashdata('message', array('type' => 'success', 'message' => 'Tender Save successfully'));
            redirect('srrp/tender_entry/' . $data['id']);
        } else {
            $this->session->set_flashdata('message', array('type' => 'warning', 'message' => 'can\'t save now, please try again later'));
            // echo '<script>alert("cannot be saved now, please try again later");</script>';
        }
    ...
    function wo()
    {
        ...
        $this->data['heading'] = 'PATHASHREE-II Work Order';
    ...
    {
        $session = $this->common->get_session();
        if ($session['role_id'] == 20 || $session['role_id'] == 21) {
            $wp_list = $this->model->get_wp_list($session['user_id'], $status);
            $this->data['wp'] = json_encode($wp_list);
            $this->data['status'] = $status;
            $this->data['heading'] = 'PATHASHREE-II Work Progress List';
    ...
    function wp_entry($id, $status)
    {
        ...
        $this->data['status'] = $status;
        $this->data['heading'] = 'PATHASHREE-II Work Progress Entry';
        $this->data['subheading'] = $status == 0 ? 'Not-Started' : ($status == 5 ? 'Completed' : 'Work-In-Progress');
    ...
    function wp_save()
    {
        ...
        redirect(base_url('srrp/wp/' . $input['status']));
    }

    function wp_upload($id, $srrp_id)
    {
        $path = 'uploads/srrp/progress/';
        if (!file_exists('./' . $path)) {
            mkdir('./' . $path, 0777, true);
        }
        $config['upload_path'] = './' . $path;
        ...
        if (!$this->upload->do_upload('image1')) {
            ...
            $old_file = $path . '/' . $file['upload_data']['file_name'];
            if (strlen($old_file) > 0) {
                $new_file = $path . $srrp_id . '_' . $id . '_' . 'image1' . $file['upload_data']['file_ext'];
                rename($old_file, $new_file);
                $input = array(
                    'image1' => $new_file
                );
                ...
                $this->db->update(SRRP, $input);
                $this->db->where('id', $id);
                $this->db->update(SRRP_PROGRESS, $input);
            }
        }
        if (!$this->upload->do_upload('image2')) {
            ...
            $old_file = $path . '/' . $file['upload_data']['file_name'];
            if (strlen($old_file) > 0) {
                $new_file = $path . '/' . $srrp_id . '_' . $id . '_' . 'image2' . $file['upload_data']['file_ext'];
                rename($old_file, $new_file);
                $input = array(
                    'image2' => $new_file
                );
        ...
        if (!$this->upload->do_upload('image3')) {
            ...
            $old_file = $path . '/' . $file['upload_data']['file_name'];
            if (strlen($old_file) > 0) {
                $new_file = $path . '/' . $srrp_id . '_' . $id . '_' . 'image3' . $file['upload_data']['file_ext'];
                rename($old_file, $new_file);
                $input = array(
                    'image3' => $new_file
                );
    ...
    function qm()
    {
        ...
        $this->data['selected'] = json_encode($selected);
        $this->data['heading'] = 'PATHASHREE-II Quality Monitoring';
    ...
    function qm_details($id)
    {
        ...
        $this->data['heading'] = 'Assignment Details';
    ...
    function qm_inspection()
    {
        ...
        $this->data['sqm'] = json_encode($sqm_list);
        $this->data['selected'] = json_encode($selected);
        $this->data['heading'] = 'PATHASHREE-II SQM Inspection';
    ...
    function qm_start($id)
    {
        if ($id > 0) {
            ...
            $this->data['heading'] = 'PATHASHREE-II Inspection Images';
    ...
    function qm_save_start()
    {
        ...
        redirect(base_url('srrp/qm_inspection'));
    }
    ...
    function qm_atr()
    {
        ...
        $this->data['list'] = json_encode($list);
        $this->data['selected'] = json_encode($selected);
        $this->data['heading'] = 'PATHASHREE-II Action Taken Report';
    ...
    function qm_inspection_report()
    {
        ...
        $this->data['sqm'] = json_encode($sqm_list);
        $this->data['selected'] = json_encode($selected);
        $this->data['heading'] = 'PATHASHREE-II Inspection Report';
    ...
    function qm_inspection_report_view($id)
    {
        if ($id > 0) {
            ...
            $this->data['heading'] = 'PATHASHREE-II Inspection Report View';
    ...
    function rpt_state_summary()
    {
        ...
        $this->data['selected'] = json_encode($selected);
        $this->data['list'] = json_encode($list);
        $this->data['heading'] = 'PATHASHREE-II State Summary Report';
        $this->data['subheading'] = 'pathashree-rastashree schemes';
        $this->data['title'] = 'PATHASHREE-RASTASHREE';
    ...
    function rpt_agency_progress()
    {
        ...
        $this->data['selected'] = json_encode($selected);
        $this->data['list'] = json_encode($list);
        $this->data['heading'] = 'PATHASHREE-II Agency wise Progress';
        $this->data['subheading'] = 'pathashree-rastashree schemes';
        $this->data['title'] = 'PATHASHREE-RASTASHREE';
    ...
    function rpt_road_type_progress()
    {
        ...
        $this->data['list'] = json_encode($list);
        $this->data['selected'] = json_encode($selected);
        $this->data['heading'] = 'PATHASHREE-II Road Type wise Progress';
        $this->data['subheading'] = 'pathashree-rastashree schemes';
        $this->data['title'] = 'PATHASHREE-RASTASHREE';
    ...
    function rpt_work_type_progress()
    {
        ...
        $this->data['list'] = json_encode($list);
        $this->data['selected'] = json_encode($selected);
        $this->data['heading'] = 'PATHASHREE-II Work Type wise Progress';
        $this->data['subheading'] = 'pathashree-rastashree schemes';
        $this->data['title'] = 'PATHASHREE-RASTASHREE';
    ...
    function rpt_ps_work_status()
    {
        ...
        $this->data['selected'] = json_encode($selected);
        $this->data['list'] = json_encode($list);
        $this->data['heading'] = 'PATHASHREE-II PS wise Work Status Report';
        $this->data['subheading'] = 'pathashree-rastashree schemes';
        $this->data['title'] = 'PATHASHREE-RASTASHREE';
    ...
    function rpt_work_progress()
    {
        ...
        $this->data['selected'] = json_encode($selected);
        $this->data['list'] = json_encode($list);
        $this->data['heading'] = 'PATHASHREE-II Work wise Progress Report';
        $this->data['subheading'] = 'pathashree-rastashree schemes';
        $this->data['title'] = 'PATHASHREE-RASTASHREE';
    ...
    function rpt_work_progress_details($srrp_id)
    {
        $list = $this->model->get_rpt_work_progress_details($srrp_id);
        $this->data['list'] = sizeof($list) > 0 ? json_encode($list) : '';
        $this->data['title'] = 'Work Progress Report';
        $this->data['heading'] = 'PATHASHREE-II Work Progress Details Report';
    ...
    function rpt_qm_summary()
    {
        ...
        $this->data['district'] = json_encode($district_list);
        $this->data['heading'] = 'PATHASHREE-II Quality Monitoring Report';
        $this->data['subheading'] = 'pathashree-rastashree schemes';
        $this->data['title'] = 'PATHASHREE-RASTASHREE';
    ...
    function update()
    {
        ...
        $this->data['selected'] = json_encode($selected);
        $this->data['list'] = json_encode($list);
        $this->data['heading'] = 'PATHASHREE-II Update of Pathashree Schemes';
        $this->data['subheading'] = 'pathashree-rastashree schemes';
        $this->data['title'] = 'PATHASHREE-RASTASHREE';
    ...
    //        $this->data['selected'] = json_encode($selected);
    //        $this->data['list'] = json_encode($list);
    //        $this->data['heading'] = 'Update Summary Report';
    //        $this->data['subheading'] = 'pathashree-rastashree schemes';
    //        $this->data['title'] = 'PATHASHREE-RASTASHREE';
    ...
    function rpt_updated_work_summary()
    {
        ...
        if ($this->input->post()) {
            ...
            if (sizeof($_post) > 0) {
                ...
                $block_list = $this->common->get_block_list($selected['district_id']);
                $list = $this->model->get_rpt_updated_work_summary($selected['district_id'], $selected['block_id']);
            }
        }
        $district_list = $this->common->get_district_list();
        if (sizeof($district_list) == 1) {
        ...
        $this->data['district'] = json_encode($district_list);
        $this->data['selected'] = json_encode($selected);
        $this->data['heading'] = 'PATHASHREE-II Updated Work Summary';
        $this->data['subheading'] = 'pathashree-rastashree schemes';
        $this->data['title'] = 'PATHASHREE-RASTASHREE';
    ...
    function rpt_updated_work_details()
    {
        ...
        $this->data['district'] = json_encode($district_list);
        $this->data['selected'] = json_encode($selected);
        $this->data['heading'] = 'PATHASHREE-II Updated Work Details';
        $this->data['subheading'] = 'pathashree-rastashree schemes';
        $this->data['title'] = 'PATHASHREE-RASTASHREE';
        $this->data['content'] = $this->parser->parse('rpt_updated_work_details', $this->data, true);
        $this->parser->parse('../../templates/template.php', $this->data);
    }
    function rpt_completion()
    {
        $this->data['list'] = json_encode($this->model->get_completion_report());
        $this->data['content'] = $this->parser->parse('rpt_completion', $this->data, true);
        $this->parser->parse('../../templates/template.php', $this->data);
    }
    ...
    //     $this->data['subheading'] = 'PATHASHREE-II schemes';
    //     $this->data['title'] = 'PATHASHREE-RASTASHREE';
