...
class Common extends MX_Controller
{
    ...
    function ridf_report()
    {
        $sql = 'TRUNCATE table ridf_report';
        $this->db->query($sql);
        $sql = 'SELECT DISTINCT s.district_id, d.name as district, ac.id as ac_id, ac.name as ac, s.block_id, b.name as block, sa.name as agency, srt.name as road_type, '
            . 'swt.name as work_type FROM ridf s JOIN division d ON s.district_id=d.id JOIN division b ON s.block_id=b.id JOIN assembly_constituency as ac ON s.ac_id=ac.id '
            . 'JOIN scheme_road_type srt ON s.road_type_id=srt.id JOIN scheme_work_type swt ON s.work_type_id=swt.id JOIN scheme_agency sa ON s.agency_id=sa.id '
            . 'WHERE s.scheme_id is not null AND s.survey_status=6 AND s.isactive=1 ORDER BY d.name, s.agency, road_type, work_type';
        $query = $this->db->query($sql);
        $result = $query->result();
        foreach ($result as $row) {
            $input = array(
            ...
            $this->db->insert('ridf_report', $input);
        }
    }

    function update_ridf_report()
    {
        $sql = 'SELECT id, district_id, ac_id, block_id, agency, road_type, work_type FROM ridf_report ORDER BY id';
        $query = $this->db->query($sql);
        $result = $query->result();
        foreach ($result as $row) {
            $this->_update_ridf_scheme_info($row->id, $row->district_id, $row->ac_id, $row->block_id, $row->agency, $row->road_type, $row->work_type);
            $this->_update_ridf_approved_info($row->id, $row->district_id, $row->ac_id, $row->block_id, $row->agency, $row->road_type, $row->work_type);
            $this->_update_ridf_tender_invited_info($row->id, $row->district_id, $row->ac_id, $row->block_id, $row->agency, $row->road_type, $row->work_type);
            $this->_update_ridf_tender_te_info($row->id, $row->district_id, $row->ac_id, $row->block_id, $row->agency, $row->road_type, $row->work_type);
            $this->_update_ridf_tender_fe_info($row->id, $row->district_id, $row->ac_id, $row->block_id, $row->agency, $row->road_type, $row->work_type);
            $this->_update_ridf_tender_tm_info($row->id, $row->district_id, $row->ac_id, $row->block_id, $row->agency, $row->road_type, $row->work_type);
            $this->_update_ridf_benefitted_info($row->id, $row->district_id, $row->ac_id, $row->block_id, $row->agency, $row->road_type, $row->work_type);
            $this->_update_ridf_wo_info($row->id, $row->district_id, $row->ac_id, $row->block_id, $row->agency, $row->road_type, $row->work_type);
        }
    }

    function update_ridf_report_daily()
    {
        $sql = 'SELECT id, district_id, ac_id, block_id, agency, road_type, work_type FROM ridf_report ORDER BY id';
        $query = $this->db->query($sql);
        $result = $query->result();
        foreach ($result as $row) {
            $this->_update_ridf_progress_25_info($row->id, $row->district_id, $row->ac_id, $row->block_id, $row->agency, $row->road_type, $row->work_type);
            $this->_update_ridf_progress_50_info($row->id, $row->district_id, $row->ac_id, $row->block_id, $row->agency, $row->road_type, $row->work_type);
            $this->_update_ridf_progress_75_info($row->id, $row->district_id, $row->ac_id, $row->block_id, $row->agency, $row->road_type, $row->work_type);
            $this->_update_ridf_progress_99_info($row->id, $row->district_id, $row->ac_id, $row->block_id, $row->agency, $row->road_type, $row->work_type);
            $this->_update_ridf_progress_100_info($row->id, $row->district_id, $row->ac_id, $row->block_id, $row->agency, $row->road_type, $row->work_type);
        }
    }

    function _update_ridf_scheme_info($id, $district_id, $ac_id, $block_id, $agency, $road_type, $work_type)
    {
        $sql = 'select count(r.id) as cnt, ifnull(sum(r.length),0) as length, ifnull(sum(r.sanctioned_cost),0) as cost from ridf r '
            . 'join scheme_road_type srt on r.road_type_id=srt.id join scheme_work_type swt on r.work_type_id=swt.id join scheme_agency sa on r.agency_id=sa.id '
            . 'where r.isactive=1 and r.district_id=' . $district_id . ' and r.ac_id=' . $ac_id . ' and r.block_id=' . $block_id .
            ' and sa.name="' . $agency . '" and srt.name="' . $road_type . '" and swt.name="' . $work_type . '"';
        $query = $this->db->query($sql);
        $row = $query->row();
        $input = array(
            'scheme' => $row->cnt,
        ...
        $this->db->where('id', $id);
        $this->db->update('ridf_report', $input);
    }

    function _update_ridf_approved_info($id, $district_id, $ac_id, $block_id, $agency, $road_type, $work_type)
    {
        $sql = 'select count(r.id) as cnt, ifnull(sum(r.length),0) as length, ifnull(sum(r.sanctioned_cost),0) as cost from ridf r '
            . 'join scheme_road_type srt on r.road_type_id=srt.id join scheme_work_type swt on r.work_type_id=swt.id join scheme_agency sa on r.agency_id=sa.id '
            . 'where r.isactive=1 and r.district_id=' . $district_id . ' and r.ac_id=' . $ac_id . ' and r.block_id=' . $block_id .
    ...
    function _update_ridf_tender_invited_info($id, $district_id, $ac_id, $block_id, $agency, $road_type, $work_type)
    {
        $sql = 'select count(r.id) as cnt, ifnull(sum(r.length),0) as length, ifnull(sum(r.sanctioned_cost),0) as cost from ridf r '
            . 'join scheme_road_type srt on r.road_type_id=srt.id join scheme_work_type swt on r.work_type_id=swt.id join scheme_agency sa on r.agency_id=sa.id '
            . 'where r.survey_status=6 and r.isactive=1 and r.tender_number is not null and r.district_id=' . $district_id . ' and r.ac_id=' . $ac_id . ' and r.block_id=' . $block_id .
            ' and sa.name="' . $agency . '" and srt.name="' . $road_type . '" and swt.name="' . $work_type . '"';
        $query = $this->db->query($sql);
        $row = $query->row();
        $input = array(
    ...
    function _update_ridf_tender_te_info($id, $district_id, $ac_id, $block_id, $agency, $road_type, $work_type)
    {
        $sql = 'select count(r.id) as cnt, ifnull(sum(r.length),0) as length, ifnull(sum(r.sanctioned_cost),0) as cost from ridf r join ridf_tender_log tl on tl.ridf_id=r.id '
            . 'join scheme_road_type srt on r.road_type_id=srt.id join scheme_work_type swt on r.work_type_id=swt.id join scheme_agency sa on r.agency_id=sa.id '
            . 'where r.survey_status=6 and r.tender_number is not null and tl.evaluation_status=1 and r.isactive=1 and r.district_id=' . $district_id . ' and r.ac_id=' . $ac_id . ' and r.block_id=' . $block_id .
            ' and sa.name="' . $agency . '" and srt.name="' . $road_type . '" and swt.name="' . $work_type . '"';
        $query = $this->db->query($sql);
        $row = $query->row();
        $input = array(
    ...
    function _update_ridf_tender_fe_info($id, $district_id, $ac_id, $block_id, $agency, $road_type, $work_type)
    {
        $sql = 'select count(r.id) as cnt, ifnull(sum(r.length),0) as length, ifnull(sum(r.sanctioned_cost),0) as cost from ridf r join ridf_tender_log tl on tl.ridf_id=r.id '
            . 'join scheme_road_type srt on r.road_type_id=srt.id join scheme_work_type swt on r.work_type_id=swt.id join scheme_agency sa on r.agency_id=sa.id '
            . 'where r.survey_status=6 and r.tender_number is not null and tl.bid_opening_status=1 and r.isactive=1 and r.district_id=' . $district_id . ' and r.ac_id=' . $ac_id . ' and r.block_id=' . $block_id .
            ' and sa.name="' . $agency . '" and srt.name="' . $road_type . '" and swt.name="' . $work_type . '"';
        $query = $this->db->query($sql);
        $row = $query->row();
        $input = array(
    ...
    function _update_ridf_tender_tm_info($id, $district_id, $ac_id, $block_id, $agency, $road_type, $work_type)
    {
        $sql = 'select count(r.id) as cnt, ifnull(sum(r.length),0) as length, ifnull(sum(r.sanctioned_cost),0) as cost from ridf r join ridf_tender_log tl on tl.ridf_id=r.id '
            . 'join scheme_road_type srt on r.road_type_id=srt.id join scheme_work_type swt on r.work_type_id=swt.id join scheme_agency sa on r.agency_id=sa.id '
            . 'where r.survey_status=6 and r.tender_number is not null and tl.bid_matured_status=1 and r.isactive=1 and r.district_id=' . $district_id . ' and r.ac_id=' . $ac_id . ' and r.block_id=' . $block_id .
            ' and sa.name="' . $agency . '" and srt.name="' . $road_type . '" and swt.name="' . $work_type . '"';
        $query = $this->db->query($sql);
        $row = $query->row();
        $input = array(
    ...
    function _update_ridf_benefitted_info($id, $district_id, $ac_id, $block_id, $agency, $road_type, $work_type)
    {
        $sql = 'select count(distinct block_id) as block, 0 as gp, 0 as village, 0 as households, 0 as population from ridf r '
            . 'join scheme_road_type srt on r.road_type_id=srt.id join scheme_work_type swt on r.work_type_id=swt.id join scheme_agency sa on r.agency_id=sa.id '
            . 'where r.isactive=1 and r.district_id=' . $district_id . ' and r.ac_id=' . $ac_id . ' and r.block_id=' . $block_id .
            ' and sa.name="' . $agency . '" and srt.name="' . $road_type . '" and swt.name="' . $work_type . '"';
        $query = $this->db->query($sql);
    ...
    function _update_ridf_wo_info($id, $district_id, $ac_id, $block_id, $agency, $road_type, $work_type)
    {
        $sql = 'select count(r.id) as cnt, ifnull(sum(r.length),0) as length, ifnull(sum(wo.awarded_cost),0) as cost from ridf r join ridf_wo wo on wo.ridf_id=r.id '
            . 'join scheme_road_type srt on r.road_type_id=srt.id join scheme_work_type swt on r.work_type_id=swt.id join scheme_agency sa on r.agency_id=sa.id '
            . 'where r.survey_status=6 and r.wo_status=2 and r.isactive=1 and r.district_id=' . $district_id . ' and r.ac_id=' . $ac_id . ' and r.block_id=' . $block_id .
            ' and sa.name="' . $agency . '" and srt.name="' . $road_type . '" and swt.name="' . $work_type . '"';
        $query = $this->db->query($sql);
        $row = $query->row();
        $input = array(
    ...
    function _update_ridf_progress_25_info($id, $district_id, $ac_id, $block_id, $agency, $road_type, $work_type)
    {
        $sql = 'select count(r.id) as cnt, round(sum(r.length * r.physical_progress/100), 2) as length, ifnull(sum(wo.awarded_cost),0) as cost from ridf r join ridf_wo wo on wo.ridf_id=r.id '
            . 'join scheme_road_type srt on r.road_type_id=srt.id join scheme_work_type swt on r.work_type_id=swt.id join scheme_agency sa on r.agency_id=sa.id '
            . 'where r.survey_status=6 and r.pp_status=1 and r.isactive=1 and r.district_id=' . $district_id . ' and r.ac_id=' . $ac_id . ' and r.block_id=' . $block_id .
            ' and sa.name="' . $agency . '" and srt.name="' . $road_type . '" and swt.name="' . $work_type . '"';
        $query = $this->db->query($sql);
        $row = $query->row();
        $input = array(
    ...
    function _update_ridf_progress_50_info($id, $district_id, $ac_id, $block_id, $agency, $road_type, $work_type)
    {
        $sql = 'select count(r.id) as cnt, round(sum(r.length * r.physical_progress/100), 2) as length, ifnull(sum(wo.awarded_cost),0) as cost from ridf r join ridf_wo wo on wo.ridf_id=r.id '
            . 'join scheme_road_type srt on r.road_type_id=srt.id join scheme_work_type swt on r.work_type_id=swt.id join scheme_agency sa on r.agency_id=sa.id '
            . 'where r.survey_status=6 and r.pp_status=2 and r.isactive=1 and r.district_id=' . $district_id . ' and r.ac_id=' . $ac_id . ' and r.block_id=' . $block_id .
            ' and sa.name="' . $agency . '" and srt.name="' . $road_type . '" and swt.name="' . $work_type . '"';
        $query = $this->db->query($sql);
        $row = $query->row();
        $input = array(
    ...
    function _update_ridf_progress_75_info($id, $district_id, $ac_id, $block_id, $agency, $road_type, $work_type)
    {
        $sql = 'select count(r.id) as cnt, round(sum(r.length * r.physical_progress/100), 2) as length, ifnull(sum(wo.awarded_cost),0) as cost from ridf r join ridf_wo wo on wo.ridf_id=r.id '
            . 'join scheme_road_type srt on r.road_type_id=srt.id join scheme_work_type swt on r.work_type_id=swt.id join scheme_agency sa on r.agency_id=sa.id '
            . 'where r.survey_status=6 and r.pp_status=3 and r.isactive=1 and r.district_id=' . $district_id . ' and r.ac_id=' . $ac_id . ' and r.block_id=' . $block_id .
            ' and sa.name="' . $agency . '" and srt.name="' . $road_type . '" and swt.name="' . $work_type . '"';
        $query = $this->db->query($sql);
        $row = $query->row();
        $input = array(
    ...
    function _update_ridf_progress_99_info($id, $district_id, $ac_id, $block_id, $agency, $road_type, $work_type)
    {
        $sql = 'select count(r.id) as cnt, round(sum(r.length * r.physical_progress/100), 2) as length, ifnull(sum(wo.awarded_cost),0) as cost from ridf r join ridf_wo wo on wo.ridf_id=r.id '
            . 'join scheme_road_type srt on r.road_type_id=srt.id join scheme_work_type swt on r.work_type_id=swt.id join scheme_agency sa on r.agency_id=sa.id '
            . 'where r.survey_status=6 and r.pp_status=4 and r.isactive=1 and r.district_id=' . $district_id . ' and r.ac_id=' . $ac_id . ' and r.block_id=' . $block_id .
            ' and sa.name="' . $agency . '" and srt.name="' . $road_type . '" and swt.name="' . $work_type . '"';
        $query = $this->db->query($sql);
        $row = $query->row();
        $input = array(
    ...
    function _update_ridf_progress_100_info($id, $district_id, $ac_id, $block_id, $agency, $road_type, $work_type)
    {
        $sql = 'select count(r.id) as cnt, round(sum(r.length * r.physical_progress/100), 2) as length, ifnull(sum(wo.awarded_cost),0) as cost from ridf r join ridf_wo wo on wo.ridf_id=r.id '
            . 'join scheme_road_type srt on r.road_type_id=srt.id join scheme_work_type swt on r.work_type_id=swt.id join scheme_agency sa on r.agency_id=sa.id '
            . 'where r.survey_status=6 and r.pp_status=5 and r.isactive=1 and r.district_id=' . $district_id . ' and r.ac_id=' . $ac_id . ' and r.block_id=' . $block_id .
            ' and sa.name="' . $agency . '" and srt.name="' . $road_type . '" and swt.name="' . $work_type . '"';
        $query = $this->db->query($sql);
        $row = $query->row();
        $input = array(
    ...
    function generate_ridf_ref_no_temp()
    {
        $sql = 'select s.id, d.name, s.district_id, s.agency from ridf s join division d on s.district_id=d.id where s.id > 8317 order by district_id, agency';
        $query = $this->db->query($sql);
        $result = $query->result();
        foreach ($result as $row) {
            $ref_no = 'TMP/' . str_pad($row->district_id . rand(0, 999), 5, '0', STR_PAD_LEFT) . '/' . $row->agency . '/' . $row->name . '/PATHASHREE3/2024';
    ...
    // ######################## Rural Roads ####################
    function roads_report()
    {
        $sql = 'TRUNCATE table roads_report';
        $this->db->query($sql);
        $sql = 'SELECT DISTINCT s.district_id, d.name as district, ac.id as ac_id, ac.name as ac, s.block_id, b.name as block, s.agency, s.road_type, '
            . 's.work_type FROM roads s JOIN division d ON s.district_id=d.id JOIN division b ON s.block_id=b.id JOIN assembly_constituency as ac ON s.ac_id=ac.id '
            . 'WHERE s.ref_no is not null AND s.survey_status=6 AND s.isactive=1 ORDER BY d.name, s.agency, s.road_type, s.work_type';
        $query = $this->db->query($sql);
        $result = $query->result();
        foreach ($result as $row) {
            ...
            $this->db->insert('roads_report', $input);
        }
    }
    function update_roads_report()
    {
        $sql = 'SELECT id, district_id, ac_id, block_id, agency, road_type, work_type FROM roads_report ORDER BY id';
        $query = $this->db->query($sql);
        $result = $query->result();
        foreach ($result as $row) {
            $this->_update_roads_scheme_info($row->id, $row->district_id, $row->ac_id, $row->block_id, $row->agency, $row->road_type, $row->work_type);
            $this->_update_roads_approved_info($row->id, $row->district_id, $row->ac_id, $row->block_id, $row->agency, $row->road_type, $row->work_type);
            $this->_update_roads_tender_invited_info($row->id, $row->district_id, $row->ac_id, $row->block_id, $row->agency, $row->road_type, $row->work_type);
            $this->_update_roads_tender_te_info($row->id, $row->district_id, $row->ac_id, $row->block_id, $row->agency, $row->road_type, $row->work_type);
            $this->_update_roads_tender_fe_info($row->id, $row->district_id, $row->ac_id, $row->block_id, $row->agency, $row->road_type, $row->work_type);
            $this->_update_roads_tender_tm_info($row->id, $row->district_id, $row->ac_id, $row->block_id, $row->agency, $row->road_type, $row->work_type);
            $this->_update_roads_benefitted_info($row->id, $row->district_id, $row->ac_id, $row->block_id, $row->agency, $row->road_type, $row->work_type);
            $this->_update_roads_wo_info($row->id, $row->district_id, $row->ac_id, $row->block_id, $row->agency, $row->road_type, $row->work_type);
        }
    }

    function update_roads_report_daily()
    {
        $sql = 'SELECT id, district_id, ac_id, block_id, agency, road_type, work_type FROM roads_report ORDER BY id';
        $query = $this->db->query($sql);
        $result = $query->result();
        foreach ($result as $row) {
            $this->_update_roads_progress_25_info($row->id, $row->district_id, $row->ac_id, $row->block_id, $row->agency, $row->road_type, $row->work_type);
            $this->_update_roads_progress_50_info($row->id, $row->district_id, $row->ac_id, $row->block_id, $row->agency, $row->road_type, $row->work_type);
            $this->_update_roads_progress_75_info($row->id, $row->district_id, $row->ac_id, $row->block_id, $row->agency, $row->road_type, $row->work_type);
            $this->_update_roads_progress_99_info($row->id, $row->district_id, $row->ac_id, $row->block_id, $row->agency, $row->road_type, $row->work_type);
            $this->_update_roads_progress_100_info($row->id, $row->district_id, $row->ac_id, $row->block_id, $row->agency, $row->road_type, $row->work_type);
        }
    }

    function _update_roads_scheme_info($id, $district_id, $ac_id, $block_id, $agency, $road_type, $work_type)
    {
        $sql = 'select count(id) as cnt, ifnull(sum(proposed_length),0) as length, ifnull(sum(cost),0) as cost from roads '
            . 'where isactive=1 and district_id=' . $district_id . ' and ac_id=' . $ac_id . ' and block_id=' . $block_id .
            ' and agency="' . $agency . '" and road_type="' . $road_type . '" and work_type="' . $work_type . '"';
        $query = $this->db->query($sql);
        $row = $query->row();
        ...
        $this->db->where('id', $id);
        $this->db->update('roads_report', $input);
    }

    function _update_roads_approved_info($id, $district_id, $ac_id, $block_id, $agency, $road_type, $work_type)
    {
        $sql = 'select count(id) as cnt, ifnull(sum(approved_length),0) as length, ifnull(sum(cost),0) as cost from roads '
            . 'where survey_status=6 and isactive=1 and district_id=' . $district_id . ' and ac_id=' . $ac_id . ' and block_id=' . $block_id .
            ' and agency="' . $agency . '" and road_type="' . $road_type . '" and work_type="' . $work_type . '"';
        $query = $this->db->query($sql);
        $row = $query->row();
    ...
    function _update_roads_tender_invited_info($id, $district_id, $ac_id, $block_id, $agency, $road_type, $work_type)
    {
        $sql = 'select count(id) as cnt, ifnull(sum(approved_length),0) as length, ifnull(sum(cost),0) as cost from roads '
            . 'where survey_status=6 and isactive=1 and tender_number is not null '
            . 'and district_id=' . $district_id . ' and block_id=' . $block_id . ' and ac_id=' . $ac_id
    ...
    function _update_roads_tender_te_info($id, $district_id, $ac_id, $block_id, $agency, $road_type, $work_type)
    {
        $sql = 'select count(s.id) as cnt, ifnull(sum(approved_length),0) as length, ifnull(sum(cost),0) as cost '
            . 'from roads s join roads_tender_log tl on tl.roads_id=s.id '
            . 'where s.survey_status=6 and s.isactive=1 and s.tender_number is not null and tl.evaluation_status=1 '
            . 'and district_id=' . $district_id . ' and ac_id=' . $ac_id . ' and block_id=' . $block_id
            . ' and agency="' . $agency . '" and road_type="' . $road_type . '" and work_type="' . $work_type . '"';
        $query = $this->db->query($sql);
    ...
    function _update_roads_tender_fe_info($id, $district_id, $ac_id, $block_id, $agency, $road_type, $work_type)
    {
        $sql = 'select count(s.id) as cnt, ifnull(sum(approved_length),0) as length, ifnull(sum(cost),0) as cost '
            . 'from roads s join roads_tender_log tl on tl.roads_id=s.id where s.survey_status=6 and s.isactive=1 '
            . 'and s.tender_number is not null and tl.bid_opening_status=1 '
            . 'and district_id=' . $district_id . ' and block_id=' . $block_id . ' and ac_id=' . $ac_id
            . ' and agency="' . $agency . '" and road_type="' . $road_type . '" and work_type="' . $work_type . '"';
        $query = $this->db->query($sql);
    ...
    function _update_roads_tender_tm_info($id, $district_id, $ac_id, $block_id, $agency, $road_type, $work_type)
    {
        $sql = 'select count(distinct s.id) as cnt, ifnull(sum(approved_length),0) as length, ifnull(sum(cost),0) as cost '
            . 'from roads s join roads_tender_log tl on tl.roads_id=s.id where s.survey_status=6 and s.isactive=1 '
            . 'and s.tender_number is not null and tl.bid_matured_status=1 '
    ...
    function _update_roads_benefitted_info($id, $district_id, $ac_id, $block_id, $agency, $road_type, $work_type)
    {
        $sql = 'select count(distinct block_id) as block, count(distinct gp_id) as gp, ifnull(SUM(no_of_village),0) as village, '
            . 'ifnull(SUM(total_households),0) as households, ifnull(SUM(total_population),0) as population '
            . 'from roads where survey_status=6 and isactive=1 and district_id=' . $district_id . ' and ac_id=' . $ac_id . ' and block_id=' . $block_id .
            ' and agency="' . $agency . '" and road_type="' . $road_type . '" and work_type="' . $work_type . '"';
        $query = $this->db->query($sql);
        $row = $query->row();
        $input = array(
    ...
    function _update_roads_wo_info($id, $district_id, $ac_id, $block_id, $agency, $road_type, $work_type)
    {
        $sql = 'select count(s.id) as cnt, ifnull(sum(s.approved_length),0) as length, ifnull(sum(wo.awarded_cost),0) as cost '
            . 'from roads s join roads_wo wo on wo.roads_id=s.id where s.survey_status=6 and s.isactive=1 and s.wo_status=2 '
            . 'and district_id=' . $district_id . ' and ac_id=' . $ac_id . ' and block_id=' . $block_id . ' and ac_id=' . $ac_id . ' and agency="' . $agency . '" and road_type="' . $road_type . '" and work_type="' . $work_type . '"';
        $query = $this->db->query($sql);
        $row = $query->row();
        $input = array(
    ...
    function _update_roads_progress_25_info($id, $district_id, $ac_id, $block_id, $agency, $road_type, $work_type)
    {
        $sql = 'select count(s.id) as cnt, round(sum((s.length*s.physical_progress)/100), 2) as length, sum(wo.awarded_cost) as cost '
            . 'from roads s join roads_wo as wo on wo.roads_id=s.id where s.survey_status=6 and s.isactive=1 and s.pp_status=1 '
            . 'and s.district_id=' . $district_id . ' and ac_id=' . $ac_id . ' and s.block_id=' . $block_id .
            ' and s.agency="' . $agency . '" and s.road_type="' . $road_type . '" and s.work_type="' . $work_type . '"';
        $query = $this->db->query($sql);
        $row = $query->row();
    ...
    function _update_roads_progress_50_info($id, $district_id, $ac_id, $block_id, $agency, $road_type, $work_type)
    {
        $sql = 'select count(s.id) as cnt, round(sum((s.length*s.physical_progress)/100), 2) as length, sum(wo.awarded_cost) as cost '
            . 'from roads s join roads_wo as wo on wo.roads_id=s.id where s.survey_status=6 and s.isactive=1 and s.pp_status=2 '
            . 'and s.district_id=' . $district_id . ' and ac_id=' . $ac_id . ' and s.block_id=' . $block_id .
            ' and s.agency="' . $agency . '" and s.road_type="' . $road_type . '" and s.work_type="' . $work_type . '"';
        $query = $this->db->query($sql);
        $row = $query->row();
    ...
    function _update_roads_progress_75_info($id, $district_id, $ac_id, $block_id, $agency, $road_type, $work_type)
    {
        $sql = 'select count(s.id) as cnt, round(sum((s.length*s.physical_progress)/100), 2) as length, sum(wo.awarded_cost) as cost '
            . 'from roads s join roads_wo as wo on wo.roads_id=s.id where s.survey_status=6 and s.isactive=1 and s.pp_status=3 '
            . 'and s.district_id=' . $district_id . ' and s.ac_id=' . $ac_id . ' and s.block_id=' . $block_id
            . ' and s.agency="' . $agency . '" and s.road_type="' . $road_type . '" and s.work_type="' . $work_type . '"';
        $query = $this->db->query($sql);
        $row = $query->row();
    ...
    function _update_roads_progress_99_info($id, $district_id, $ac_id, $block_id, $agency, $road_type, $work_type)
    {
        $sql = 'select count(s.id) as cnt, round(sum((s.length*s.physical_progress)/100), 2) as length, sum(wo.awarded_cost) as cost '
            . 'from roads s join roads_wo as wo on wo.roads_id=s.id where s.survey_status=6 and s.isactive=1 and s.pp_status=4 '
            . 'and s.district_id=' . $district_id . ' and ac_id=' . $ac_id . ' and s.block_id=' . $block_id .
            ' and s.agency="' . $agency . '" and s.road_type="' . $road_type . '" and s.work_type="' . $work_type . '"';
        $query = $this->db->query($sql);
        $row = $query->row();
    ...
    function _update_roads_progress_100_info($id, $district_id, $ac_id, $block_id, $agency, $road_type, $work_type)
    {
        $sql = 'select count(s.id) as cnt, round(sum((s.approved_length*s.physical_progress)/100), 2) as length, sum(wo.awarded_cost) as cost '
            . 'from roads s join roads_wo as wo on wo.roads_id=s.id where s.survey_status=6 and s.isactive=1 and s.pp_status=5 '
            . 'and s.district_id=' . $district_id . ' and s.ac_id=' . $ac_id . ' and s.block_id=' . $block_id .
            ' and s.agency="' . $agency . '" and s.road_type="' . $road_type . '" and s.work_type="' . $work_type . '"';
        $query = $this->db->query($sql);
        $row = $query->row();
    ...
    function generate_roads_ref_no_temp()
    {
        $sql = 'select s.id, d.name, s.district_id, s.agency from roads s join division d on s.district_id=d.id where s.id > 8317 order by district_id, agency';
        $query = $this->db->query($sql);
        $result = $query->result();
        foreach ($result as $row) {
            $ref_no = 'TMP/' . str_pad($row->district_id . rand(0, 999), 5, '0', STR_PAD_LEFT) . '/' . $row->agency . '/' . $row->name . '/RURALROADS/2025';
            $input = array(
                'ref_no' => $ref_no
            );
            $this->db->where('id', $row->id);
