...
class Ws extends REST_Controller {
    ...
    function api_post() {
        ...
        switch ($code) {
            ...
            case WS_SRRP_SCHEME_COUNT:
                $this->_srrp_scheme_count($data);
                break;
            case WS_SRRP_LIST:
                $this->_srrp($data);
                break;
            case WS_SRRP_WO_DETAILS:
                $this->_srrp_wo_details($data);
                break;
            case WS_DIVISION:
                $this->_division($data);
                break;
            case WS_SAVE_SRRP:
                $this->_save_srrp($data);
                break;
            default:
                break;
        }
   ...
   function _srrp_scheme_count($data) {
        $data = $data[0];
        $arr = $this->ws_model->get_srrp_scheme_count($data);
        $this->response([
            'status' => TRUE,
            'message' => 'success',
            'data' => $arr
   ...
   function _srrp($data) {
        $data = $data[0];
        $arr = $this->ws_model->get_srrp_list($data);
        $this->response([
            'status' => TRUE,
            'message' => 'success',
            'data' => $arr
   ...
   function _srrp_wo_details($data) {
        $data = $data[0];
        $arr = $this->ws_model->get_srrp_wo_details($data);
        $this->response([
            'status' => TRUE,
            'message' => 'success',
            'data' => $arr
   ...
   function _division($data) {
    ...
    function _save_srrp($data) {
        $data = $data[0];
        $id = $this->ws_model->save_srrp($data);
        $message = 'Data saved successfully ...';
        if($id==-2){
    ...
    function api_upload_post() {
        ...
        $issrrp = isset($data->issrrp) && $data->issrrp ? true : false;
        $path = $issrrp ? 'uploads/srrp/progress':'uploads/inspection';
