                            ...
                            <table id="tbl_requisition" class="display nowrap table table-bordered table-striped table-hover" style="width:100%;">
                                ...
                                <tbody>
                                    ...
                                    foreach ($list as $row) {
                                        ...
                                        $preview = in_array($role_id, [1, 2, 25]) || strlen($row->ref_no) > 0 && strlen($row->claimed_doc) == 0 ? '<a href="' . base_url('ridf/requisition_preview/' . $row->ridf_id . '/' . $row->id) . '" target="_blank" title="preview of requisition"><i class="fa fa-file-pdf fa-2x pointer" style="color: red;"></i></a>' : '';
                                        $final_submission = strlen($row->ref_no) > 0 && strlen($row->claimed_doc) == 0 ? '<a href="' . base_url('ridf/requisition_claim/' . $row->ridf_id . '/' . $row->id) . '" target="_self" title="upload signed copy of claimed document"><i class="fa fa-upload fa-2x pointer" style="color: grey;"></i>' : '';
                                        ...
                                        echo '<td align="center"><p style="margin:0px; width: 80px">' . $preview . '&nbsp;&nbsp;' . $final_submission . '&nbsp;&nbsp;<a href="' . base_url('ridf/requisition_entry/' . $row->ridf_id) . '" target="_self"><button class="btn btn-icon btn-round btn-sm btn-primary" title="add/edit"><i class="fas fa-pen pointer"></i></button></a></p></td>';
                                        echo '</tr>';
                                        $i++;
                                    }
                                    ?>
