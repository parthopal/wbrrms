...
class RIDF extends MX_Controller
{
    ...
    function qm()
    {
        ...
        $this->data['list'] = json_encode($list);
        $this->data['selected'] = json_encode($selected);
        $this->data['heading'] = 'RIDF - Quality Monitoring';
        ...
        if ($session['role_id'] == 4) {
            ...
            if ($this->input->post()) {
                ...
                if (sizeof($_post) > 0) {
                    ...
                    $scheme_list = $this->model->get_qm_scheme_list($selected['district_id'], $selected['sqm_id'], $selected['month'], $selected['year']);
                }
            }
            $district_list = $this->common->get_district_list();
    ...
    function requisition_approved()
    {
        ...
        $list = $this->model->get_curr_requisition_approved_list($selected);
        $district_list = $this->common->get_district_list();
        $this->data['role_id'] = $session['role_id'];
        $this->data['district'] = json_encode($district_list);
        $this->data['category'] = json_encode($this->model->get_category_list());
        ...
        $this->data['heading'] = 'Requisition - RIDF';
        $this->data['subheading'] = 'Requisition';
        $this->data['content'] = $this->parser->parse('requisition_approved', $this->data, true);
        $this->parser->parse('../../templates/template.php', $this->data);
    }


    function requisition_approve_entry($ridf_id, $id)
    {
        $work_info = $this->model->get_work_info($ridf_id);
        ...
        $this->data['work'] = json_encode($work_info);
        $this->data['curr'] = json_encode($curr_requisition_info);
        $this->data['title'] = 'Upload requisition approved';
        $this->data['heading'] = 'Requisition approved - RIDF';
        $this->data['subheading'] = 'UPload requisition approved';
        $this->data['content'] = $this->parser->parse('requisition_approve_entry', $this->data, true);
        $this->parser->parse('../../templates/template.php', $this->data);
    }


    function requisition_approved_save()
    {
        $data = $this->input->post();
        // print_r($data);exit;
        $id = $this->model->requisition_approved_save($data);
        if ($id > 0) {
        ...
        redirect('ridf/requisition_approved');
    }


    function requisition_final_approval($ridf_id, $id)
    {
        $work_info = $this->model->get_work_info($ridf_id);
        $curr_requisition_info = $this->model->get_curr_requisition_info($id);
        $this->data['id'] = $id;
        $this->data['work'] = json_encode($work_info);
        $this->data['curr'] = json_encode($curr_requisition_info);
        $this->data['title'] = 'Upload requisition approval';
        $this->data['heading'] = 'Requisition approval - RIDF';
        $this->data['subheading'] = 'UPload requisition approval';
        $this->data['content'] = $this->parser->parse('requisition_final_approval', $this->data, true);
        $this->parser->parse('../../templates/template.php', $this->data);
    }

    function requisition_final_approval_save()
    {
        $data = $this->input->post();
        // print_r($data);exit;
        $id = $data['id'];
        if ($id > 0) {
            $this->upload_final_approval($id);
            $this->session->set_flashdata('message', array('type' => 'success', 'message' => 'Data saved successfully'));
        } else {
            $this->session->set_flashdata('message', array('type' => 'danger', 'message' => 'Data can not be saved now, please try later or contact with support team.'));
        }
    ...
    function upload_final_approval($id)
    {
        $path = 'uploads/ridf/requisition/';
        if (!file_exists('./' . $path)) {
            mkdir('./' . $path, 0777, true);
        ...
        if (!$this->upload->do_upload('approved_doc')) {
            ...
            $image = array('upload_data' => $this->upload->data());
            $old_file = $path . $image['upload_data']['file_name'];
            $new_file = $path . $id . '_final_approval' . $image['upload_data']['file_ext'];
            rename($old_file, $new_file);
            $input = array(
                'approved_doc' => $new_file
            );
    ...
    function requisition_approve_preview($ridf_id, $id)
    {
        $session = $this->common->get_session();
        $work_info = $this->model->get_work_info($ridf_id, $id);
        $total_info = $this->model->get_total_requisition_info($ridf_id);
        ...
        $this->data['heading'] = 'Requisition - RIDF';
        $this->data['subheading'] = 'Requisition';
        $this->data['content'] = $this->parser->parse('requisition_approve_preview', $this->data, true);
        $this->parser->parse('../../templates/preview.php', $this->data);
    }
