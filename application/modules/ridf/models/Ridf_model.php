...
class RIDF_Model extends CI_Model
{
    ...
    function get_inspection_list_bridge($sqm_id, $month, $year)
    {
    ...
    function get_inspection_report_list_bridge($sqm_id, $month, $year)
    {
        $where = $sqm_id != '' ? ' AND qm.sqm_id=' . $sqm_id : '';
    ...
    function get_inspection_caption_bridge($qm_id)
    {
    ...
    function get_selection_bridge($qm_id)
    {
        $sql = 'SELECT s.name, s.agency FROM ridf_bridge_qm as qm JOIN ridf s ON qm.ridf_id=s.id WHERE qm.id=' . $qm_id;
        $query = $this->db->query($sql);
        return $query->result();
    ...
    function get_qm_list($month, $year)
    {
        ...
        $sql = 'SELECT CONCAT(u.id, "-", qm.month, "-", qm.year) as id, u.name as sqm, u.mobile,
            GROUP_CONCAT(DISTINCT d.name) as district, COUNT(ridf_id) as total
            FROM ridf_qm as qm JOIN division d ON qm.district_id=d.id JOIN ridf s ON s.id=qm.ridf_id JOIN user u ON u.id=qm.sqm_id
    ...
    function get_qm_scheme_list($district_id, $sqm_id = 0, $month = 0, $year = 0)
    {
        ...
        $sql = 'SELECT s.id, d.name as district, b.name as block, s.name, s.agency, s.length, s.wo_start_date, s.physical_progress, qm.inspection_date, qm.overall_grade, ifnull(qm.atr_action, 2) as atr_action, total.cnt, ifnull(q.id, 0) as selected
            FROM ridf s JOIN division d ON s.district_id=d.id JOIN division b ON s.block_id=b.id
            LEFT JOIN ridf_qm q ON s.id=q.ridf_id and q.sqm_id=' . $sqm_id . ' and q.month=' . $month . ' and q.year=' . $year . ' and q.isactive=1
            LEFT JOIN (SELECT sq.ridf_id, sqi.inspection_date, sqi.overall_grade, sqi.atr_action FROM ridf_qm sq JOIN ridf_qm_inspection sqi ON sqi.qm_id=sq.id WHERE sqi.inspection_date=(SELECT max(inspection_date) from ridf_qm_inspection as sqi1 join ridf_qm sq1 on sqi1.qm_id=sq1.id WHERE sq1.ridf_id=sq.ridf_id)) as qm ON qm.ridf_id=s.id
            LEFT JOIN (SELECT DISTINCT ridf_id, count(ridf_id) as cnt FROM ridf_qm GROUP BY ridf_id) as total ON total.ridf_id=s.id
    ...
    function qm_save($data)
    {
        ...
        foreach ($data['chk'] as $row) {
            ...
            // var_dump($input);exit;
            $query = $this->db->get(RIDF_QM);
            if ($query->num_rows() > 0) {
                $this->db->where('id', $query->row()->id);
                $this->db->update(RIDF_QM, $input);
            } else {
                $input['created'] = date('Y-m-d');
                $this->db->insert(RIDF_QM, $input);
            }
        }
        $this->db->trans_complete();
        if ($this->db->trans_status() === FALSE) {
    ...
    {
        $session = $this->common->get_session();
        $where = $session['district_id'] > 0 ? ' and r.district_id=' . $session['district_id'] : '';
    ...
    //         . 'sum(ifnull(rr.approved_amt,0)) as approved_amt, sum(ifnull(rr.expenditure_amt,0)) as expenditure_amt, sum(ifnull(rr.dpr_amt,0)) as dpr, sum(ifnull(rr.contigency_amt,0)) as contigency, SUM(ifnull(approved_claimed_amt,0)) AS total_approved_claimed_amt,SUM(ifnull(approved_dpr_amt,0)) AS total_approved_dpr_amt, SUM(ifnull(approved_contigency_amt,0)) AS total_approved_contigency_amt');
    //     $this->db->where(array(
    //         'r.id' => $ridf_id,
    //         'wo.isactive' => 1
    //     ));
    ...
    //     $this->db->group_by('d.name, b.name, c.subcategory, r.scheme_id, r.name, a.name, r.length, r.unit, r.sanctioned_cost, wo.awarded_cost, rr.ridf_id');
    //     $query = $this->db->get(RIDF . ' r');
    //     return $query->row();
    // }
    ...
    function get_work_info($ridf_id, $id = 0)
    {
        ...
        r.sanctioned_cost,
        wo.awarded_cost,
        MAX(rr.physical_progress) as physical_progress,
        SUM(IFNULL(rr.approved_amt, 0)) as approved_amt,
        SUM(IFNULL(rr.expenditure_amt, 0)) as expenditure_amt,
        SUM(IFNULL(rr.dpr_amt, 0)) as dpr,
        SUM(IFNULL(rr.contigency_amt, 0)) as contigency,
        SUM(IFNULL(rr.approved_claimed_amt, 0)) AS total_approved_claimed_amt,
        SUM(IFNULL(rr.approved_dpr_amt, 0)) AS total_approved_dpr_amt,
        SUM(IFNULL(rr.approved_contigency_amt, 0)) AS total_approved_contigency_amt,
        SUM(ifnull(claimed_expenditure_amt,0)) AS total_claimed_expenditure_amt, 
        SUM(ifnull(dpr_expenditure_amt,0)) AS total_dpr_expenditure_amt, 
        SUM(ifnull(contigency_expenditure_amt,0)) AS total_contigency_expenditure_amt
    ');
        ...
        if ($id > 0) {
            ...
            $this->db->join(RIDF_REQUISITION . ' rr', 'rr.ridf_id = r.id', 'left');
        }
    ...
    {
        $this->db->select('ra_id, ifnull(ref_no, "") as ref_no, memo_no, date_format(memo_date, "%d/%m/%Y") as memo_date, physical_progress, claimed_amt, dpr_amt, contigency_amt, approved_claimed_amt,approved_dpr_amt, approved_contigency_amt');
        $this->db->where('id', $id);
    ...
    function get_curr_requisition_approved_list($selected)
    {
        $where = $selected['district_id'] > 0 ? ' AND r.district_id=' . $selected['district_id'] : '';
        $where .= $selected['category_id'] > 0 ? ' AND r.category_id=' . $selected['category_id'] : '';
        $sql = 'SELECT DISTINCT rr.id, c.subcategory as category, d.name as district, rr.ridf_id, r.name, rr.ra_id, ifnull(rr.ref_no, "") as ref_no, rr.memo_no, '
            ...
            . 'date_format(rr.approved_date, "%d/%m/%Y") as approved_date,rr.approved_claimed_amt,rr.approved_dpr_amt,rr.approved_contigency_amt, rr.claimed_expenditure_amt, rr.dpr_expenditure_amt, rr.contigency_expenditure_amt	, ifnull(rr.claimed_doc, "") as claimed_doc, '
            . 'ifnull(rr.approved_doc, "") as approved_doc FROM ridf_requisition rr JOIN ridf r ON rr.ridf_id=r.id JOIN division d ON r.district_id=d.id '
            . 'JOIN scheme_category c ON rr.category_id=c.id WHERE rr.isactive = 1' . $where . ' AND rr.ra_id = (SELECT max(ra_id) FROM ridf_requisition WHERE rr.ridf_id=ridf_id GROUP BY ridf_id) AND rr.claimed_doc IS NOT NULL '
            . 'ORDER BY d.name, c.subcategory, r.name, rr.ra_id';
        $query = $this->db->query($sql);
        return $query->result();
    }
    ...
    function requisition_approved_save($data)
    {
        $id = $data['id'];
        $input = array(
            'approved_date' => date('Y-m-d', strtotime(str_replace('/', '-', $data['approved_date']))),
            'approved_claimed_amt' => $data['approved_claimed_amt'],
            'approved_dpr_amt' => $data['approved_dpr_amt'],
            'approved_contigency_amt' => $data['approved_contigency_amt']
        );
        $this->db->where('id', $id);
        $this->db->update('ridf_requisition', $input);
        return true;
