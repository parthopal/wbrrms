        ...
        buttons: [
            {
                ...
                title: 'RIDF MASTER ON ' + String(currentdate.getDate()).padStart(2, '0') + '/' + String((currentdate.getMonth() + 1)).padStart(2, '0') + '/'
...
function get_rpt_qm_list() {
    $.ajax({
        url: baseURL + '/capex/get_rpt_qm_list',
        type: 'get',
...
function remove(id) {
    if (confirm('Do you want to remove this scheme?') === true) {
        $.ajax({
            url: baseURL + '/capex/remove_survey_list',
            type: 'get',
...
function edit(id) {
    window.location.href = baseURL + '/capex/entry/' + id;
}
function get_survey_pending_list() {
    if ($('#district_id').val() > 0 && $('#block_id').val() > 0) {
        $.ajax({
            url: baseURL + '/capex/get_survey_pending_list',
            type: 'get',
            data: {
...
function back_to_inbox(id) {
    window.location.href = baseURL + '/capex/back_to_inbox/' + id;
}

function mark_not_traceable(id) {
    window.location.href = baseURL + '/capex/not_traceable/' + id;
}

function add_survey(id) {
    window.location.href = baseURL + '/capex/survey_entry/' + id;
}
function vec(id) {
    ...
    if (isNaN(cost)) {
        ...
        $('#cost_' + id).text(cost);
        $.ajax({
            url: baseURL + '/capex/survey_vec_save',
            type: 'get',
...
function print_lot() {
    window.open(baseURL + '/capex/print_lot/' + $('#district_id').val() + '/' + $('#lotno').val(), '_blank');
}
function get_lot_list() {
    if ($('#district_id').val() > 0 && $('#lotno').val().length > 0) {
        $.ajax({
            url: baseURL + '/capex/get_lot_list',
            type: 'get',
...
function get_approval_list() {
    if ($('#district_id').val() > 0) {
        $.ajax({
            url: baseURL + '/capex/get_approval_list',
            type: 'get',
            data: {
                district_id: $('#district_id').val(),
                block_id: $('#block_id').val(),
...
function get_approved_list() {
    if ($('#district_id').val() > 0) {
        $.ajax({
            url: baseURL + '/capex/get_approved_list',
            type: 'get',
            data: {
                district_id: $('#district_id').val(),
...
function get_state_approval_list() {
    if ($('#district_id').val() > 0 && $('#block_id').val() > 0) {
        $.ajax({
            url: baseURL + '/capex/get_state_approval_list',
            type: 'get',
            data: {
                district_id: $('#district_id').val(),
                block_id: $('#block_id').val(),
...
function get_not_imp_list() {
    $.ajax({
        url: baseURL + '/capex/get_not_imp_list',
        type: 'get',
        data: {
            district_id: $('#district_id').val(),
...
function delete_not_traceable(id) {
    ...
    if (r === true) {
        window.location.href = baseURL + '/capex/delete_not_traceable/' + id;
    }
}
...
function get_wo_list() {
    $.ajax({
        ...
        type: 'get',
        data: { district_id: $('#district_id').val(), category_id: $('#category_id').val(), type_id: $('#type_id').val() },
        dataType: 'json',
        async: false
    }).done(function (data) {
        _load_wo_list(data);
    ...
    $('#tbl').DataTable({
        ...
        columnDefs: [
            ...
            },
            {
                targets: 12, defaultContent: '',
                render: function (data, type, full, meta) {
                    return '<button class="btn btn-icon btn-round btn-sm btn-primary" onclick="wo_add(' + full.capex_id + ')"  title="Edit"><i class="fas fa-pen pointer"></i></button>'
                }
            }
        ]
    });
        ...
        buttons: [
            {
                extend: 'excel',
                text: 'Excel',
                filename: 'capex_bridge_list_' + $.now(),
                title: 'RIDF BRIDGE LIST ON ' + String(currentdate.getDate()).padStart(2, '0') + '/' + String((currentdate.getMonth() + 1)).padStart(2, '0') + '/'
                    + currentdate.getFullYear() + ' ' + String(currentdate.getHours()).padStart(2, '0') + ':' + String(currentdate.getMinutes()).padStart(2, '0'),
                footer: true,
                exportOptions: {
                    columns: ':not(.not-export)'
...
function bridge_edit(id) {
    window.location.href = baseURL + '/capex/bridge_entry/' + id;
}
function bridge_remove(id) {
    var r = confirm('Do you want to delete this?')
    if (r === true) {
        $.ajax({
            url: baseURL + '/capex/bridge_remove',
            type: 'post',
            data: { id: id },
            dataType: 'json',
            async: false
