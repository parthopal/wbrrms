...
//    var form = document.querySelector('form');
        ...
        $.ajax({
            ...
            window.location.href = baseURL + '/sf/lot';
        });
    });
...
function get_survey_list() {
    if ($('#district_id').val() > 0) {
        $.ajax({
            url: baseURL + '/sf/get_survey_list',
            type: 'get',
...
function get_rpt_qm_list() {
    $.ajax({
        url: baseURL + '/sf/get_rpt_qm_list',
        type: 'get',
...
function remove(id) {
    if (confirm('Do you want to remove this scheme?') === true) {
        $.ajax({
            url: baseURL + '/sf/remove_survey_list',
            type: 'get',
...
function add(id) {
    window.location.href = baseURL + '/sf/entry/' + id;
}
function get_survey_pending_list() {
    if ($('#district_id').val() > 0 && $('#block_id').val() > 0) {
        $.ajax({
            url: baseURL + '/sf/get_survey_pending_list',
            type: 'get',
            data: {
...
function mark_not_traceable(id) {
    window.location.href = baseURL + '/sf/not_traceable/' + id;
}

function add_survey(id) {
    window.location.href = baseURL + '/sf/survey_entry/' + id;
}
function vec(id) {
    ...
    if (isNaN(cost)) {
        ...
        $('#cost_' + id).text(cost);
        $.ajax({
            url: baseURL + '/sf/survey_vec_save',
            type: 'get',
...
//        $.ajax({
//            url: baseURL + '/sf/forwarded_to_dm',
//            type: 'get',
//            data: {id: id, district_id: $('#district_id').val(), block_id: $('#block_id').val()},
//            dataType: 'json',
//            async: false
...
//}
function print_lot() {
    window.open(baseURL + '/sf/print_lot/' + $('#lotno').val(), '_blank');
}
function get_lot_list() {
    if ($('#district_id').val() > 0 && $('#lotno').val().length > 0) {
        $.ajax({
            url: baseURL + '/sf/get_lot_list',
            type: 'get',
...
function get_approval_list() {
    if ($('#district_id').val() > 0) {
        $.ajax({
            url: baseURL + '/sf/get_approval_list',
            type: 'get',
            data: {
                district_id: $('#district_id').val(),
                block_id: $('#block_id').val(),
...
function _load_approval_list(data) {
    ...
    $('#tbl').DataTable({
        ...
        columnDefs: [
            ...
            {targets: 7, data: 'proposed_length'},
...
function get_approved_list() {
    if ($('#district_id').val() > 0) {
        $.ajax({
            url: baseURL + '/sf/get_approved_list',
            type: 'get',
            data: {
                district_id: $('#district_id').val(),
                block_id: $('#block_id').val(),
...
function get_state_approval_list() {
    if ($('#district_id').val() > 0 && $('#block_id').val() > 0) {
        $.ajax({
            url: baseURL + '/sf/get_state_approval_list',
            type: 'get',
            data: {
                district_id: $('#district_id').val(),
                block_id: $('#block_id').val(),
...
function get_not_imp_list() {
    $.ajax({
        url: baseURL + '/sf/get_not_imp_list',
        type: 'get',
        data: {
            district_id: $('#district_id').val(),
...
function delete_not_traceable(id) {
    ...
    if (r === true) {
        window.location.href = baseURL + '/sf/delete_not_traceable/' + id;
    }
}
...
function tender_entry(id) {
    window.location.href = baseURL + '/sf/tender_entry/' + id;
}

function get_tender_list() {
    if ($('#district_id').val() > 0) {
        $.ajax({
            url: baseURL + '/sf/get_tender_list',
            type: 'get',
            data: {
                district_id: $('#district_id').val(),
                block_id: $('#block_id').val(),
    ...
    $('#tbl').DataTable({
        ...
        columnDefs: [
            ...
            },
            {
                ...
                render: function (data, type, full, meta) {
                    return (
                            '<p style="margin:0px; width: 80px"><button class="btn btn-icon btn-round btn-sm btn-primary" onclick="wo_add(' +
                            full.sf_id + ')"  title="Edit"><i class="fas fa-pen pointer"></i></button></p>'
                            );
                }
            }
            // {targets: 11, render: function (data, type, full, meta) {
            //         return '<button class="btn btn-icon btn-round btn-sm btn-primary" onclick="wo_add(' + full.sf_id + ')"  title="Edit"><i class="fas fa-pen pointer"></i></button>'
            //     }}
        ]
    });
}
...
function wo_add(id) {
    window.location.href = baseURL + '/sf/wo_entry/' + id;
}
function wo_remove(id) {
    var r = confirm('Do you want to delete this?')
    if (r === true) {
        $.ajax({
            url: baseURL + '/sf/wo_remove',
            type: 'get',
            data: {id: id},
            dataType: 'json',
            async: false
