...
function get_work_info() {
    ...
    if (ridf_id > 0) {
        ...
        $.ajax({
            ...
            $('#info_5').html(Math.round(data.approved_amt) > 0 ? Math.round(data.approved_amt) - Math.round(data.dpr) - Math.round(data.contigency) : 0);
            $('#info_6').html(Math.round(data.expenditure_amt));
            $('#info_7').html(Math.round(data.approved_amt) - Math.round(data.expenditure_amt));
            var loan = (data.sanctioned_cost * 0.8).toFixed(0);
            $('#info_8').html(loan);
            $('#info_9').html(parseInt(data.dpr) + '/' + (loan * 0.005).toFixed(0));
            $('#info_10').html(parseInt(data.contigency) + '/' + (data.awarded_cost * 0.03).toFixed(0));
        });
    } else {
        $('#info').hide();
    }
